...
// Validation et correction de métadonnées
export function validateProductMeta(meta) {
  const errors = [];
  const warnings = [];
  
  // Validation densité
  if (meta.density_g_per_ml < 0.1 || meta.density_g_per_ml > 2.0) {
    errors.push(`Densité suspecte: ${meta.density_g_per_ml} g/ml`);
  }
  if (meta.confidence_density < 0.7) {
    warnings.push(`Faible confiance sur la densité (${(meta.confidence_density * 100).toFixed(0)}%)`);
  }
  
  // Validation poids par unité
  if (meta.grams_per_unit !== null) {
    if (meta.grams_per_unit < 1 || meta.grams_per_unit > 5000) {
      errors.push(`Poids par unité suspect: ${meta.grams_per_unit} g`);
    }
    if (meta.confidence_unit < 0.7) {
      warnings.push(`Faible confiance sur le poids unitaire (${(meta.confidence_unit * 100).toFixed(0)}%)`);
    }
  }
  ...
  return {
    ...
    corrections: {
      density_g_per_ml: meta.density_g_per_ml < 0.1 ? 1.0 : meta.density_g_per_ml > 2.0 ? 1.0 : meta.density_g_per_ml,
      grams_per_unit: meta.grams_per_unit && (meta.grams_per_unit < 1 || meta.grams_per_unit > 5000) ? null : meta.grams_per_unit
    }
  };
}

// Recherche intelligente de produits existants
export function findBestProductMatch(query, productCatalog) {
  ...
  return productCatalog.map(product => {
    ...
    else if (normalizedName.includes(normalizedQuery)) score += 800;
    else if (normalizedQuery.includes(normalizedName)) score += 700;
    
    // Score par mots
    queryWords.forEach(word => {
      if (word.length < 2) return;
      
      if (normalizedName.includes(word)) score += 100;
      if (normalizedCategory.includes(word)) score += 50;
      
      // Score de proximité (Levenshtein simplifié)
      const nameWords = normalizedName.split(/\s+/);
      nameWords.forEach(nameWord => {
        const similarity = 1 - (levenshteinDistance(word, nameWord) / Math.max(word.length, nameWord.length));
        if (similarity > 0.7) score += similarity * 80;
      });
    });
    
    // Bonus pour les métadonnées complètes
    if (product.density_g_per_ml && product.density_g_per_ml !== 1.0) score += 10;
    if (product.grams_per_unit) score += 10;
    if (product.category && product.category !== 'Autre') score += 5;
...
// Estimation intelligente de la durée de conservation (en jours)
export function estimateShelfLife({ name = '', category = '' }) {
  const text = `${name} ${category}`.toLowerCase();
  
  // Produits très périssables (1-3 jours)
  if (/viande\s+(hachée|crue)|poisson\s+frais|fruits?\s+de\s+mer|abats/i.test(text)) return 2;
  if (/salade|épinards?|roquette|mâche|herbes?\s+fraîches?/i.test(text)) return 3;
  
  // Produits périssables (3-7 jours)
  if (/viandes?|volailles?|charcuteries?\s+fraîches?/i.test(text)) return 5;
  if (/yaourts?|crèmes?\s+fraîches?|fromages?\s+frais/i.test(text)) return 7;
  if (/tomates?\s+cerises?|fraises|framboises|myrtilles/i.test(text)) return 4;
  
  // Produits semi-périssables (7-14 jours)
  if (/laits?|œufs?|beurres?/i.test(text)) return 10;
  if (/légumes?\s+(verts?|feuilles?)|brocolis?|choux?\s+fleurs?/i.test(text)) return 7;
  if (/bananes?|avocats?|pêches|abricots/i.test(text)) return 8;
  
  // Fruits et légumes de garde (14-30 jours)
  if (/pommes?(?!\s+de\s+terre)|poires?|oranges?|citrons?/i.test(text)) return 21;
  if (/carottes?|oignons?|pommes?\s+de\s+terre|courges?/i.test(text)) return 30;
  
  // Fromages à pâte dure (30-60 jours)
  if (/fromages?\s+(durs?|à\s+pâte\s+dure|gruyère|comté)/i.test(text)) return 45;
  
  // Produits secs et conserves (365+ jours)
  if (/conserves?|bocaux|farines?|riz|p[aâ]tes|légumineuses/i.test(text)) return 365;
  if (/huiles?|vinaigres?|miels?|sucres?|sels?|épices?/i.test(text)) return 730;
  
  // Surgelés (90-180 jours)
  if (/surgelés?|congelés?|glacés?/i.test(text)) return 120;
  
  // Par défaut pour produits frais
  return 7;
}

// Suggestion d'unité par défaut intelligente
export function suggestDefaultUnit({ name = '', category = '', estimated_meta = null }) {
  const text = `${name} ${category}`.toLowerCase();
  
  // Liquides → volume
  if (/laits?|jus|huiles?|vinaigres?|sauces?|sirops?|crèmes?\s+liquides?/i.test(text)) return 'ml';
  
  // Produits naturellement comptés en unités
  if (/œufs?|fruits?(?!\s+(secs?|à\s+coque))|légumes?\s+(entiers?|pièces?)/i.test(text)) return 'u';
  if (/pommes?|poires?|bananes?|oranges?|citrons?|tomates?|oignons?|carottes?/i.test(text)) return 'u';
  
  // Herbes et épices → souvent en petites quantités
  if (/herbes?|épices?|aromates?|condiments?/i.test(text)) return 'g';
  
  // Utilisation des métadonnées estimées
  if (estimated_meta?.grams_per_unit && estimated_meta.confidence_unit > 0.7) return 'u';
  if (estimated_meta?.density_g_per_ml && estimated_meta.density_g_per_ml < 1.1 && estimated_meta.confidence_density > 0.8) return 'ml';
  
  // Par défaut → grammes (plus universel)
  return 'g';
}

// Export des constantes pour usage externe si nécessaire
export const DENSITY_DATABASE = DENSITY_RULES;
export const UNIT_WEIGHT_DATABASE = UNIT_WEIGHT_RULES;
export const CATEGORY_DATABASE = CATEGORY_RULES;
