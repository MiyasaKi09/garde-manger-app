...
import { normalizeMeatMealsPerWeek } from '@/lib/domain/settings/planningSettings'
...
export async function updateMember(supabase, id, patch = {}) {
  ...
  // Le quota carné (livrable 1.1) est le premier réglage de planning que le
  // moteur lit comme un NOMBRE de repas. Il est validé ici, au dernier point
  // avant l'écriture : `mergePlanningPreferences` le fait dans le domaine, mais
  // aucune route ne l'appelait — l'écran envoie ses préférences telles quelles.
  // Une saisie hors bornes doit être refusée, pas ramenée en silence à une
  // valeur qui se relirait comme un choix. Aucun autre champ n'est touché :
  // normaliser tout le bloc `planning` ici écrirait des valeurs par défaut dans
  // des profils qui n'en portent pas.
  const planning = allowed.preferences?.planning
  if (planning && typeof planning === 'object' && 'meat_meals_per_week' in planning) {
    const quota = normalizeMeatMealsPerWeek(planning.meat_meals_per_week)
    allowed.preferences = {
      ...allowed.preferences,
      planning: quota == null
        ? Object.fromEntries(Object.entries(planning).filter(([key]) => key !== 'meat_meals_per_week'))
        : { ...planning, meat_meals_per_week: quota },
    }
  }
  const { data, error } = await supabase
