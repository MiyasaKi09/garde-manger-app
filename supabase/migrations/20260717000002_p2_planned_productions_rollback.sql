-- Myko P2 — Rollback : suppression de planned_productions et planned_consumptions.
-- Version : 20260717000002_rollback
--
-- Ordre d'exécution imposé par les contraintes FK :
--   1. Drop des politiques RLS (avant DROP TABLE)
--   2. DROP TABLE planned_consumptions (FK planned_production_id → planned_productions)
--   3. DROP TABLE planned_productions
--   4. Restauration de publish_closed_loop_plan à sa version P1
--
-- (transaction fournie par apply-migrations.sh --single-transaction)

-- ═══════════════════════════════════════════════════════════════════════════
-- 1. Drop des politiques RLS (avant toute suppression de table)
-- ═══════════════════════════════════════════════════════════════════════════

-- planned_consumptions
DROP POLICY IF EXISTS planned_consumptions_select_own ON public.planned_consumptions;
DROP POLICY IF EXISTS planned_consumptions_insert_own ON public.planned_consumptions;
DROP POLICY IF EXISTS planned_consumptions_update_own ON public.planned_consumptions;
DROP POLICY IF EXISTS planned_consumptions_delete_own ON public.planned_consumptions;

-- planned_productions
DROP POLICY IF EXISTS planned_productions_select_own ON public.planned_productions;
DROP POLICY IF EXISTS planned_productions_insert_own ON public.planned_productions;
DROP POLICY IF EXISTS planned_productions_update_own ON public.planned_productions;
DROP POLICY IF EXISTS planned_productions_delete_own ON public.planned_productions;

-- ═══════════════════════════════════════════════════════════════════════════
-- 2. Drop tables (dépendances FK respectées : consumptions avant productions)
-- ═══════════════════════════════════════════════════════════════════════════

DROP TABLE IF EXISTS public.planned_consumptions;
DROP TABLE IF EXISTS public.planned_productions;

-- ═══════════════════════════════════════════════════════════════════════════
-- 3. Restauration de publish_closed_loop_plan — version P1
--    Corps identique à 20260717000001_p1_cooked_dish_reservations.sql §3.
--    Supprime la gestion des productions[], consumptions[] et la validation
--    explicite des orphelins de dépendances (conservée par contrat P1 implicite).
-- ═══════════════════════════════════════════════════════════════════════════
...
COMMENT ON FUNCTION public.publish_closed_loop_plan(jsonb) IS
  'Publication atomique d''un plan de boucle fermée (P1 — après rollback P2).
   Ingère slots, issues, snapshots, réservations (lots ET plats cuisinés),
   tâches, dépendances et courses dans la MÊME transaction.
   Les productions et consommations planifiées (P2) ont été supprimées.';
