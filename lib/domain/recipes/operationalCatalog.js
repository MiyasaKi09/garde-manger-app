...
// Même raccord pour le profil de conservation : la base ne le porte pas encore,
// et sans lui le planificateur ne produirait plus aucune portion d'avance en
// production. On le lit dans le corpus versionné, par code de recette — la
// même source que le catalogue canonique. Un profil que la base déclarerait un
// jour l'emporte ; à défaut, le corpus ; à défaut, null (et aucune production).
const conservationByCode = new Map((corpus.recipes || [])
  .map((recipe) => [recipe.code, conservationProfileFromCorpus(recipe.conservation_profile)]))

function operationalConservationProfile(record) {
  if (record?.conservationProfile && typeof record.conservationProfile === 'object') return record.conservationProfile
  return conservationByCode.get(record?.code) ?? null
}
...
export function materializeOperationalRecipe(record, { servings, quantityScale = 1 } = {}) {
  ...
  return {
    ...
    conservationProfile: operationalConservationProfile(record),
