...
function reheatInstruction(slot, recipe) {
  const portions = Number(slot.dishPortions) || 1
  const name = slot.cookedDishName || recipe.family
  return `Réchauffer « ${name} » (${String(portions).replace('.', ',')} portion${portions > 1 ? 's' : ''}) et servir.`
}
...
function reheatPreparation(slot, recipe, memberVariants) {
  return {
    ...
    exact_steps: [{ n: 1, instruction: reheatInstruction(slot, recipe) }],
...
export function buildCanonicalPlanPayload({
  ...
  const slots = plan.slots.map((slot) => {
    ...
    // Un créneau nourri par un plat cuisiné ne consomme AUCUN ingrédient : la
    // nourriture existe déjà, rien ne part aux besoins ni aux courses.
  ...
  const tasks = plan.slots.map((slot) => {
    ...
    if (slot.cookedDishId != null) {
      ...
      return {
        ...
        priority: 80,
        instructions: [{ n: 1, instruction: reheatInstruction(slot, recipe) }],
      }
    }
