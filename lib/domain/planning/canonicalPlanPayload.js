...
import { blocTempsPublie } from './cookingTime'
import { buildFinalDemandModel } from './finalDemands'
import { buildPresenceIndex, slotAbsence } from './mealPresence'
import { DEFAULT_HOUSEHOLD_TIME_ZONE, nextMondayIsoInTimeZone, zonedDateTimeToUtc } from './planningTime'
import { PLANNING_RULES_VERSION } from './planInvariants'
...
/**
 * Les quatorze créneaux principaux de la semaine — et ce que la présence
 * déclarée en retire (livrable 1.5).
 *
 * `presence` porte les lignes de `public.meal_presence` et `members` la liste
 * du foyer. Deux effets, et deux seulement :
 *   — chaque créneau reçoit `absentMembers`, la liste des personnes DÉCLARÉES
 *     absentes ; le solveur continue de choisir le plat du foyer, et c'est
 *     `buildPersonalizedMeals` qui n'émettra pas leur assiette ;
 *   — un créneau où TOUT LE MONDE est déclaré absent n'est plus un créneau : il
 *     disparaît de la grille. Le garder ferait cuisiner un plat que personne ne
 *     mange, avec ses courses et sa tâche de préparation.
 *
 * Sans déclaration — ou sans liste de membres, auquel cas on ne peut pas dire
 * « tout le monde » — la grille rendue est identique à celle d'avant ce
 * livrable, objet pour objet : aucune clé n'est ajoutée à un créneau sans
 * absence.
 */
export function buildWeekSlots(windowStart, { presence = [], members = [] } = {}) {
  ...
  return Array.from({ length: 7 }).flatMap((_, dayIndex) => {
    ...
    return ['dejeuner', 'diner'].flatMap((mealType) => {
      const slot = { key: `${date}-${mealType}`, date, mealType }
      if (!index.size) return [slot]
      const { absentMembers, everyoneAway } = slotAbsence({ index, members, date, mealType })
      if (everyoneAway) return []
      return [absentMembers.length ? { ...slot, absentMembers } : slot]
    })
  })
}
...
const PLAN_ISSUE_MESSAGES = {
  ...
  daily_portion_coupling_relaxed: 'Les portions d’un même plat ne peuvent être maintenues à un facteur raisonnable entre les membres ; la semaine est à revoir.',
  ...
  repetition_rules_softened: 'Les délais de retour n’ont pas pu être tenus par le corpus disponible ; le socle des règles (même jour, repas contigus, seconde cuisson, lignée) est en revanche préservé.',
  repetition_rules_relaxed: 'Aucune semaine ne respecte le socle des règles de répétition avec le corpus disponible : la proposition est à revoir.',
  // Enveloppe budgétaire (§budget) — bloquant, comme un relâchement de règle :
  // l'utilisateur a réglé un plafond et la semaine ne le tient pas. Le montant
  // et la couverture voyagent dans `details`, jamais dans le message : la
  // phrase du contrat (« estimation », « au moins », la date du référentiel)
  // est composée une seule fois, par la couche prix.
  budget_envelope_exceeded: 'L’enveloppe budgétaire de la semaine n’a pas pu être tenue : l’estimation la dépasse.',
  ...
  // Plafonds du livrable 3.1 (P2, P3, P13) — AVERTISSEMENTS. Tant que la France
  // tient plus de la moitié du vivier, ces plafonds ne refusent aucun plat : ils
  // rapportent le dépassement. La phrase le dit, pour qu'on ne lise pas un
  // blocage là où il n'y en a pas, ni un simple détail là où il y a un
  // dépassement mesuré. Les codes `starch_cap_*` et `cuisine_cap_*` portent le
  // nom de la famille et sont traités par `normalizePlanIssues`.
  dairy_egg_protein_cap: 'Trop de repas de la semaine ont un laitier ou un œuf pour protéine principale.',
}

/** Libellés des familles de féculent, pour que l'avertissement nomme ce qui déborde. */
const STARCH_LABEL = {
  pates: 'de pâtes',
  riz: 'de riz',
  pomme_de_terre: 'de pommes de terre',
  pain: 'de pain',
  semoule: 'de semoule',
  quinoa: 'de quinoa',
  legumineuses: 'de légumineuses',
  patate_douce: 'de patate douce',
  polenta: 'de polenta',
}
...
function nutritionIssues(daily, weeklyMicronutrients, demands) {
  const issues = []
  // Émettons l'issue « couplage relâché » UNE fois par jour où le drapeau est
  // levé — les entrées `daily` sont par (membre × jour), or `coupling_relaxed`
  // est une décision du foyer. Sans dédup, on aurait deux issues identiques
  // sur les mêmes journées et les mêmes clés de demande.
  const couplingRelaxedDates = new Set()
  for (const day of daily) {
    ...
    if (day.coupling_relaxed === true && !couplingRelaxedDates.has(day.meal_date)) {
      couplingRelaxedDates.add(day.meal_date)
      // Blocker : le solveur n'a pas pu maintenir le rapport de portions
      // sous le plafond `portion_ratio_cap`. Une paire de membres reçoit
      // donc des parts trop dissymétriques d'un même plat — c'est la seule
      // situation qui casse le contrat « on peut manger plus, pas autre
      // chose ». Le statut passe à `review_required`.
      issues.push({
        severity: 'blocker',
        code: 'daily_portion_coupling_relaxed',
        meal_date: day.meal_date,
        portion_ratio_cap: day.portion_ratio_cap,
        portion_ratio_lunch: day.portion_ratio_lunch,
        portion_ratio_dinner: day.portion_ratio_dinner,
        demand_keys: (demands || []).filter((demand) => demand.meal_date === day.meal_date).map((demand) => demand.demand_key),
      })
    }
...
export function normalizePlanIssues(issues = []) {
  return issues.map((issue = {}) => {
    ...
    // Plafonds du livrable 3.1 : le code porte la famille de féculent ou la
    // cuisine, parce qu'un « plafond dépassé » anonyme ne dit pas quoi corriger.
    // La phrase la reprend telle quelle quand elle ne connaît pas son libellé
    // français : mieux vaut « la cuisine "tex mex" » que rien du tout.
    const starchCap = code.startsWith('starch_cap_') ? code.slice('starch_cap_'.length) : null
    const cuisineCap = code.startsWith('cuisine_cap_') ? code.slice('cuisine_cap_'.length) : null
    const message = String(issue.message || '').trim()
      || PLAN_ISSUE_MESSAGES[code]
      || (starchCap
        ? `Trop de repas de la semaine sont servis avec le même féculent (${STARCH_LABEL[starchCap] || starchCap}).`
        : null)
      || (cuisineCap
        ? `Une même cuisine (${cuisineCap}) occupe une trop grande part de la semaine.`
        : null)
      || (repeatedProtein
...
export function buildCanonicalPlanPayload({
  ...
  // Déclarations de `public.meal_presence` pour la fenêtre (livrable 1.5).
  presence = [],
}) {
  ...
  const personalized = buildPersonalizedMeals({
    plan, recipes, members: memberList, goals, constraints, preservedMeals,
    // Les plats cuisinés que le solveur n'a pas placés restent disponibles pour
    // la famille « reste compatible » des prises support (§13).
    cookedDishes, existingDishReservations,
    presence,
    // Les sept jours de la fenêtre : un jour dont les deux prises principales
    // ont disparu (tout le foyer absent) garde son petit-déjeuner et sa
    // collation pour qui ne s'est pas déclaré absent.
    windowDates: Array.from({ length: 7 }, (_, index) => addDays(windowStart, index)),
  })
  ...
  // « Reste compatible » (§13) : une prise support nourrie par un plat cuisiné
  // référence sa source comme n'importe quel créneau. Sans cette ligne, la
  // portion serait réservée sans jamais être reliée au repas qui la mange.
  consumptions.push(...finalModel.supportLeftovers.map((leftover) => ({
    slot_key: leftover.slot_key,
    source: { cooked_dish_id: leftover.cooked_dish_id },
  ...
  const inputSnapshot = {
    ...
    // Présence déclarée (livrable 1.5). Elle entre dans l'empreinte d'entrée :
    // deux semaines qui ne diffèrent que par une absence déclarée ne sont pas
    // la même semaine, et `input_hash` doit le dire. Clé ABSENTE quand rien
    // n'est déclaré — l'empreinte d'un foyer qui n'a rien déclaré ne bouge pas.
    ...(presence?.length ? {
      presence: [...presence]
        .map((row) => ({
          household_member_id: row.household_member_id ?? row.householdMemberId ?? null,
          person_name: row.person_name ?? row.personName ?? null,
          meal_date: row.meal_date ?? row.date ?? null,
          meal_type: row.meal_type ?? row.mealType ?? null,
          present: row.present ?? null,
        }))
        .sort((gauche, droite) => JSON.stringify(gauche).localeCompare(JSON.stringify(droite))),
    } : {}),
  ...
  return {
    ...
    // Marque la version des règles (§18, lot 9) : une semaine publiée avant la
    // refonte porte l'ancienne valeur et ne doit pas être jugée à cette aune.
    rules_version: PLANNING_RULES_VERSION,
    ...
    validation_summary: {
      ...
      // P6, tel que le moteur l'a mesuré : le quota carné déclaré de chaque
      // personne, ce qu'elle reçoit, et l'écart. Recopié tel quel — le
      // recalculer ici donnerait deux chiffres qui pourraient diverger, et
      // c'est `personalizedMeals` qui a pris la décision.
      meat_quotas: personalized.meatQuotas,
      // LA PRÉSENCE (livrable 1.5), en une ligne par personne : ses assiettes
      // principales servies, la grille dont elles sont tirées, ses absences
      // déclarées, et le total nutritionnel de SA semaine — additionné sur les
      // demandes publiées, donc sur les mêmes lignes que la liste de courses.
      // Les trois effets de l'absence se lisent ainsi au même endroit.
      presence: (personalized.presence || []).map((ligne) => {
        const nutrition = (finalModel.weeklyNutritionByPerson || [])
          .find((item) => item.person_name === ligne.person_name) || null
        return {
          ...ligne,
          weekly_nutrition: nutrition?.nutrition || null,
          weekly_support_takes: nutrition?.support_takes ?? null,
        }
      }),
      shopping_item_count: shoppingItems.length,
      // LE TEMPS DE LA SEMAINE, SOUS SES TROIS DÉFINITIONS (livrable 2.4).
      // Des NOMBRES seulement : le nom et la phrase de chaque définition
      // vivent dans `cookingTime.js` (`DEFINITIONS_TEMPS`) et l'écran les y
      // lit par identifiant. Une valeur `null` veut dire « non calculable »,
      // et `manquants` dit alors ce qui manque, recette par recette — jamais
      // un total amputé présenté comme un total.
      cooking_time: blocTempsPublie({ slots: effectivePlan.slots, recipeByCode }),
      // LA CAPACITÉ DE CUISINE QUI A BORNÉ CETTE SEMAINE (livrable 2.2), telle
      // que le solveur l'a résolue : jours de session, soirs rapides, conflits
      // tranchés, et les bornes qui en découlent. Clé ABSENTE quand rien n'est
      // déclaré — même doctrine que le budget, et le plan d'un foyer silencieux
      // reste celui d'avant. Recopiée du plan, jamais recalculée : deux calculs
      // de la même borne pourraient diverger.
      ...(effectivePlan.capacity ? { cooking_capacity: effectivePlan.capacity } : {}),
    },
    slots,
    issues,
