-- Myko — matérialisation atomique des productions planifiées.
-- Version : 20260719000001 · atomic_planned_task_materialization
--
-- Garanties :
--   * une tâche, ses déductions de lots, ses mouvements de stock, ses plats
--     produits, ses réservations consommées et son état final sont modifiés dans
--     UNE transaction PostgreSQL ;
--   * les quantités réservées en grammes sont reconverties dans l'unité native du
--     lot avant décrément (g, kg, mg, ml, cl, l, unité) ;
--   * aucune production ne peut être matérialisée si le snapshot du créneau
--     contient encore des pénuries non réservées ;
--   * dépendances, propriété du tenant, idempotence et réouverture sont vérifiées
--     côté base, sans chaîne de compensations PostgREST.
...
BEGIN
  ...
  -- Réouverture : interdite après matérialisation physique ou lorsqu'une tâche
  -- dépendante est déjà terminée.
  IF NOT p_done THEN
    IF EXISTS (
      SELECT 1
  ...
  -- Idempotence stricte : une seconde validation ne recrée ni plat ni mouvement.
  IF v_task.done THEN
  ...
  -- Une tâche productrice ne peut pas fabriquer un plat abstrait alors que le
  -- planning déclare encore des ingrédients manquants. Après les courses, la
  -- semaine doit être recalculée afin de réserver les nouveaux lots.
  IF v_production_count > 0 AND v_task.task_type = 'prepare_recipe' THEN
    ...
    -- Une réservation est exprimée dans reserved_unit (actuellement souvent g),
    -- tandis que qty_remaining est dans l'unité native du lot. Chaque ligne est
    -- reconvertie avant le verrou et le décrément effectif.
  ...
  -- Nutrition par portion dérivée du snapshot immuable du créneau producteur.
