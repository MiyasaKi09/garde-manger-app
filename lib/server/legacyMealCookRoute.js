 ...
 * published). Retourne aussi l'identité canonique nécessaire aux restes.
...
async function resolveActiveSlot(supabase, userId, mealDate, mealType) {
  try {
    const { data: slots } = await supabase
      ...
      .select('id, cooked_dish_id, plan_version_id, recipe_execution_id, preparation')
      .eq('user_id', userId)
    ...
    return {
      slotId: slot.id,
      cooked_dish_id: slot.cooked_dish_id ?? null,
      planVersionId: slot.plan_version_id,
      recipeExecutionId: slot.recipe_execution_id ?? null,
...
export async function POST(request) {
  ...
  const {
    slotId,
    cooked_dish_id: slotDishId,
    planVersionId,
    recipeExecutionId,
    recipeCode,
  } = await resolveActiveSlot(
  ...
  if (!effectiveDishId && Number.isFinite(prepared) && prepared > portionsEaten) {
    ...
    const { data: created, error: leftoverErr } = await supabase
      ...
      .insert({
        ...
        canonical_recipe_code: recipeCode || null,
        canonical_recipe_execution_id: recipeExecutionId || null,
        source_plan_version_id: planVersionId || null,
