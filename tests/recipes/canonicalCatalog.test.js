...
describe('canonical recipe catalog V3', () => {
  it('publishes only fully executable recipes', () => {
    ...
    expect(recipes).toHaveLength(165)
  ...
  it('keeps all 309 candidates auditable without exposing blockers', () => {
    expect(getCanonicalRecipes({ eligibleOnly: false })).toHaveLength(356)
    expect(canonicalCatalogMetadata).toMatchObject({ recipeCount: 356, eligibleCount: 165 })
  })
