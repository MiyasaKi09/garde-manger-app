...
// Epoch Excel : 30/12/1899 (système 1900, bug année bissextile inclus)
const EXCEL_EPOCH_MS = Date.UTC(1899, 11, 30)

/**
 * Convertit une Date JS (exceljs lit les dates en UTC) en numéro de série Excel,
 * comme le faisait xlsx avec cellDates désactivé.
 */
function dateToSerial(date) {
  return (date.getTime() - EXCEL_EPOCH_MS) / 86400000
}

/**
 * Convertit un numéro de série Excel en { d, m, y } (équivalent XLSX.SSF.parse_date_code).
 */
...
/**
 * Normalise une valeur de cellule exceljs vers ce que produisait
...
function normalizeCellValue(value) {
  ...
  if (typeof value === 'object') {
    // Texte enrichi : { richText: [{ text }, ...] } — les segments de mise en
    // forme se concatènent sans séparateur ; on neutralise les retours ligne
    // internes (Alt+Entrée) qui sinon coupent un mot (« citro\nn » → « citron »
    // après collapse) et cassent le rattachement d'ingrédient.
    if (Array.isArray(value.richText)) {
      return value.richText.map(r => r.text).join('').replace(/\s+/g, ' ').trim()
    }
    // Formule : { formula | sharedFormula, result }
    if (value.formula !== undefined || value.sharedFormula !== undefined) {
      return normalizeCellValue(value.result === undefined ? null : value.result)
    }
    // Erreur de cellule (#N/A, #REF!, ...) — xlsx ne renvoyait pas de valeur exploitable
    if (value.error !== undefined) return null
    // Hyperlien : { text, hyperlink }
    if (value.hyperlink !== undefined) {
      return value.text != null ? normalizeCellValue(value.text) : null
    }
...
/**
 * Équivalent de XLSX.utils.sheet_to_json(ws, { header: 1, defval: null }) :
 * tableau de lignes (index 0), chaque ligne = tableau de cellules (index 0),
 * cellules vides → null, lignes vides conservées.
 */
function getRows(worksheet) {
  ...
  for (let r = 1; r <= rowCount; r++) {
    ...
    worksheet.getRow(r).eachCell({ includeEmpty: false }, (cell, colNumber) => {
      arr[colNumber - 1] = normalizeCellValue(cell.value)
    })
    rows.push(arr)
  }
  return rows
 ...
 * @param {ArrayBuffer|Uint8Array|Buffer} buffer - Le contenu du fichier .xlsx
 * @param {string} fileName - Nom du fichier (pour deviner l'annee)
 * @returns {Promise<{ meals, dailyTotals, batchRecipes, prepTasks, shoppingItems, meta, warnings }>}
 */
export async function parseWorkbook(buffer, fileName) {
  ...
  const meta = { sheetNames, fileName }
