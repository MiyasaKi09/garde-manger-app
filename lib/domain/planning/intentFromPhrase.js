 ...
 *                (`:1231`, `:1253`, `:1272`).
 *
 * L'ÉCLIPSE DE `maxMinutes`, ET COMMENT ELLE EST LEVÉE (livrable 4.2).
 * Le livrable 4.1 laissait ici une réserve mesurée, et elle portait sur ce seul
 * champ des cinq :
 *   1. `generate-v3/route.js` écrivait `maxMinutesByMeal: { dejeuner: 120,
 *      diner: 240 }` EN DUR et ne lisait aucun `maxTotalMinutes` du corps.
 ...
 *      Ajouter `maxTotalMinutes` aux contraintes n'aurait donc eu AUCUN effet
 *      sur les déjeuners ni les dîners — sans erreur, sans trace.
 * `plafondsParPrise` (au bas de ce fichier) est le raccord : il ne se contente
 * pas d'ajouter une clé, il ABAISSE les deux plafonds par prise à la minute
 * demandée. `generate-v3` l'appelle à la place du littéral, et
 * `tests/planning/confirmationContraintes.test.js` rejoue
 * `violatesHardConstraints` dans les deux sens pour que l'éclipse ne puisse pas
 * revenir en silence.
 *
 ...
 */
export function nombreDeclare(valeur) {
 ...
 *
 * ─────────────────────────────────────────────────────────────────────────
 * LE SOUS-ENSEMBLE ACCEPTÉ, ET POURQUOI CE FICHIER S'Y TIENT (relecture de
 * la phase 4). La sortie contrainte ne lit PAS tout JSON Schema. Sa
 * documentation énumère ce qu'elle accepte, et ce qu'elle refuse par une
 * 400 : « If you use an unsupported feature, you'll receive a 400 error ».
 * La première écriture de ce schéma en portait TREIZE :
 *
 *   — cinq unions de types (`type: ['number', 'null']`…). La documentation
 *     les donne en exemple de ce qu'il NE FAUT PAS écrire, et renvoie à
 *     `anyOf` ;
 *   — six contraintes numériques (`exclusiveMinimum`, `minimum`, `maximum`) ;
 *   — un `maxItems` (seul `minItems` à 0 ou 1 passe) ;
 *   — un `pattern`.
 *
 * Chacune aurait fait échouer la requête. Aucun test ne pouvait le voir : ils
 * remplacent tous le SDK par une doublure qui accepte n'importe quel
 * paramètre. C'est pour cela que `tests/planning/intentDepuisPhrase.test.js`
 * porte désormais un test qui PARCOURT ce schéma et refuse tout mot-clé hors
 * du sous-ensemble : la doublure ne sait pas dire non, le test le sait.
 *
 * OÙ SONT PASSÉES LES BORNES. Dans les `description`, c'est-à-dire là où les
 * SDK les déplacent eux-mêmes quand ils transforment un schéma natif — et
 * elles n'ont jamais été appliquées ailleurs que dans `validerIntention`, qui
 * est la seule porte. Le schéma dit au modèle ce qu'on attend ; il ne l'a
 * jamais garanti.
 *
 * `pattern` est remplacé par `format: 'date'`, qui dit la même chose et fait
 * partie des formats de chaîne acceptés.
 */
export function schemaDeSortie() {
  const ouNul = (schema) => ({ anyOf: [schema, { type: 'null' }] })
  return {
    type: 'object',
    ...
    properties: {
      presence: {
        ...ouNul({
          ...
          items: {
            ...
            properties: {
              ...
              date: { type: 'string', format: 'date', description: 'Jour de la semaine demandée, au format AAAA-MM-JJ' },
        ...
        description: `Déclarations de présence, ${PRESENCE_MAX} au maximum, ou null`,
      },
      starchCap: {
        ...ouNul({ type: 'number' }),
        description: "Part maximale des créneaux servis avec un même féculent, supérieure à 0 et au plus 1. 'moins de pâtes' ≈ 0.15",
      },
      meatQuota: {
        ...ouNul({ type: 'integer' }),
        description: `Nombre maximal de repas carnés de la semaine pour le foyer, entier de 0 à ${MAX_MEAT_MEALS_PER_WEEK}`,
      },
      maxMinutes: {
        ...ouNul({ type: 'integer' }),
        description: `Préparation + cuisson maximale d'un plat, en minutes, entier de ${MAX_MINUTES_BORNES.min} à ${MAX_MINUTES_BORNES.max}`,
      },
      intent: {
        ...ouNul({ type: 'string', enum: [...INTENTS_MOTEUR] }),
        description: 'Intention de la semaine, ou null',
      },
    },
  }
}
...
/**
 * LES PLAFONDS PAR PRISE ÉCRITS PAR LA ROUTE DE GÉNÉRATION — le raccord qui
 * lève l'éclipse décrite en tête de ce fichier.
 *
 * Ces deux nombres vivaient en dur dans `generate-v3/route.js`. Ils y restaient
 * même quand le foyer demandait « rien au-delà de 30 minutes », et comme
 * `closedLoopPlanner.js:936` lit la clé par prise AVANT `maxTotalMinutes`, la
 * demande ne mordait sur aucun des quatorze créneaux.
 */
export const PLAFONDS_PAR_PRISE_DEFAUT = Object.freeze({ dejeuner: 120, diner: 240 })

/**
 * La minute demandée par le corps d'une requête, ou un refus nommé.
 *
 * MÊME GARDE QUE POUR LA SORTIE DU MODÈLE, ET POUR LA MÊME RAISON : le moteur
 * fait `Number(...)` sur cette grandeur. `true` y vaudrait 1 minute — une
 * semaine vide —, `''` et `[]` y vaudraient 0. Un corps de requête est une
 * source qui se trompe exactement comme un modèle ; il passe donc la même
 * porte, et `null` (« la requête n'en parle pas ») se distingue d'une valeur
 * malformée, qui est REFUSÉE.
 *
 * @returns {{valeur: number|null, refus: object|null}}
 */
export function maxMinutesDemande(valeur) {
  if (valeur == null) return { valeur: null, refus: null }
  const resultat = validerEntier('maxTotalMinutes', valeur, MAX_MINUTES_BORNES)
  return { valeur: resultat.valeur, refus: resultat.refus[0] || null }
}

/**
 * Les plafonds par prise, ABAISSÉS à la minute demandée.
 *
 * Trois propriétés, et chacune est éprouvée :
 *   — sans demande (`null`), les plafonds sont EXACTEMENT ceux d'avant ce
 *     livrable : une génération qui ne parle pas de temps ne change pas ;
 *   — avec une demande, chaque prise descend à `min(défaut, demande)` — jamais
 *     au-dessus, parce qu'une demande ne doit pas SERVIR à desserrer une borne
 *     que le foyer n'a pas discutée ;
 *   — une valeur qui n'est pas un nombre (le piège de `Number()`) laisse les
 *     défauts intacts plutôt que de produire `{ dejeuner: 1, diner: 1 }`. La
 *     route refuse ces valeurs avant d'arriver ici ; cette garde est la
 *     seconde serrure, pas la première.
 */
export function plafondsParPrise(maxTotalMinutes, defauts = PLAFONDS_PAR_PRISE_DEFAUT) {
  const base = { ...defauts }
  const demande = nombreDeclare(maxTotalMinutes)
  if (demande == null || demande < MAX_MINUTES_BORNES.min || demande > MAX_MINUTES_BORNES.max) return base
  return Object.fromEntries(Object.entries(base).map(([prise, plafond]) => {
    const borne = nombreDeclare(plafond)
    return [prise, borne == null ? demande : Math.min(borne, demande)]
  }))
}
