...
function snackTemplates(dayIndex, target) {
  ...
  return highProteinTarget
    ? [proteinRotation[dayIndex % proteinRotation.length], egg, standard]
...
function safeSupportTemplate(candidates, constraints) {
  return candidates.find((candidate) => candidate?.items?.every((item) => supportItemAllowed(item, constraints))) || null
}
...
export function buildPersonalizedMeals({ plan, recipes, members, goals = [], constraints = {}, preservedMeals = [] }) {
  ...
  for (const [dayIndex, date] of dates.entries()) {
    ...
    for (const member of memberList) {
      ...
      const breakfastBase = rules.breakfast ? safeSupportTemplate([
        breakfastTemplate(dayIndex),
        ...Array.from({ length: 7 }, (_, offset) => breakfastTemplate(dayIndex + offset + 1)),
      ], constraints) : null
      const snackBase = rules.snack ? safeSupportTemplate(snackTemplates(dayIndex, target), constraints) : null
