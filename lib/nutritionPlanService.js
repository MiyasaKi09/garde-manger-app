...
import { declaredAbsentTakeCount } from '@/lib/domain/planning/mealPresence'

/**
 * Attache `short_description` à chaque repas dont le `canonical_recipe_code`
 * figure dans `descriptionMap`.
 *
 * Fonction pure — testable sans Supabase. L'appelant construit la map via
 * l'RPC `get_recipe_short_descriptions` puis transmet ici.
 *
 * @param {object[]} meals — lignes nutrition_plan_meals enrichies
 * @param {Map<string, string>} descriptionMap — code (majuscules) → description courte
 * @returns {object[]} — nouvelles lignes avec `short_description` ajouté si disponible
 */
export function enrichMealsWithShortDescriptions(meals, descriptionMap) {
  if (!descriptionMap || descriptionMap.size === 0) return meals
  return meals.map((meal) => {
    const code = meal.canonical_recipe_code
    if (!code) return meal
    const desc = descriptionMap.get(code.toUpperCase())
    return desc ? { ...meal, short_description: desc } : meal
  })
}
...
export async function getImport(supabase, importId) {
  ...
  const [{ data: activeVersion }, { data: householdMembers }, { data: planIssues }] = await Promise.all([
    activeVersionId
    ...
    // Issues du plan (lot 0) : blockers et warnings qui expliquent pourquoi une
    // semaine est marquée `review_required`. Affichées dans la bannière de l'interface
    // pour que l'utilisateur comprenne ce qui manque sans ouvrir les outils de dev.
    activeVersionId
      ? supabase.from('meal_plan_validation_issues').select('severity, code, message, details').eq('plan_version_id', activeVersionId).order('severity')
  ...
  if (slotIds.length) {
    // `slot_key`, `locked` et `status` sont lus pour l'épinglage (livrable
    // 3.4) : la grille doit savoir quels créneaux sont épinglés pour afficher
    // l'état du bouton, et `slot_key` est ce que les routes d'alternatives et
    // d'épinglage désignent. Trois colonnes de plus sur une requête déjà faite,
    // plutôt qu'un second aller-retour par écran.
    const { data: slots } = await supabase
      .from('meal_plan_slots')
      .select('id, slot_key, recipe_execution_id, preparation, locked, status')
      .in('id', slotIds)
  ...
  const effectiveMeals = (meals || []).map((meal) => {
    ...
    return {
      ...
      // L'épingle est portée par le CRÉNEAU, pas par la ligne personnelle :
      // deux assiettes d'un même repas ne peuvent pas être épinglées
      // séparément. `slot_locked` reste donc nul quand le créneau n'a pas été
      // lu — jamais `false`, qui voudrait dire « pas épinglé » alors qu'on ne
      // sait pas.
      slot_key: slot?.slot_key ?? null,
      slot_locked: slot ? Boolean(slot.locked) : null,
      slot_status: slot?.status ?? null,
    }
  })

  // Enrichissement des descriptions courtes — best-effort : une absence de
  // description ne casse pas la page, elle affiche juste le nom seul.
  const recipeCodes = [...new Set(effectiveMeals.map((m) => m.canonical_recipe_code).filter(Boolean))]
  let shortDescByCode = new Map()
  if (recipeCodes.length) {
    try {
      const { data: descRows } = await supabase.rpc('get_recipe_short_descriptions', { p_codes: recipeCodes })
      for (const row of descRows || []) {
        if (row.recipe_code && row.short_description) {
          shortDescByCode.set(row.recipe_code.toUpperCase(), row.short_description)
        }
      }
    } catch {
      // best-effort : RPC absente en local ou en cours de déploiement
    }
  }
  const enrichedMeals = enrichMealsWithShortDescriptions(effectiveMeals, shortDescByCode)
  ...
  if (!effectiveTotals.length && enrichedMeals.length) {
    let goals = []
    try {
    ...
    effectiveTotals = aggregateDailyTotals(enrichedMeals, goals)
  }
  ...
  // Présence déclarée sur la fenêtre (livrable 1.5). Une prise que le foyer a
  // déclaré ne pas prendre à la maison n'est pas une prise MANQUANTE : sans
  // cette soustraction, deux dîners hors domicile feraient afficher « semaine
  // incomplète — 2 prises manquantes » à qui a demandé leur retrait.
  // Repli explicite sur zéro quand la table n'existe pas encore (42P01) : la
  // page se comporte alors exactement comme avant le livrable.
  const windowDates = Array.from({ length: dayCount }, (_, index) => {
    const date = new Date(start)
    date.setUTCDate(date.getUTCDate() + index)
    return date.toISOString().slice(0, 10)
  })
  let presenceDeclarations = []
  if (windowDates.length) {
    ...
    // une erreur SQL directe `42P01`. On accepte les deux — c'est déjà la
    // convention de `lib/storageDecisionServer.js:51`. Toute AUTRE erreur
    // remonte : une lecture qui échoue n'est pas une absence de déclaration.
    if (presenceError && !['42P01', 'PGRST205'].includes(presenceError.code)) {
      throw new Error(`Présence du foyer indisponible: ${presenceError.message}`)
    }
    presenceDeclarations = presenceRows || []
  }
  const absentTakes = declaredAbsentTakeCount({
    presence: presenceDeclarations,
    members: householdMembers || [],
    dates: windowDates,
  })
  const expectedMeals = Math.max(0, expectedMealCountForWindow(householdMembers || [], dayCount) - absentTakes)
  const uniqueMealCount = new Set(enrichedMeals.map((meal) => `${meal.meal_date}|${meal.meal_type}|${meal.household_member_id || meal.person_name}`)).size
  ...
  return {
    ...
    readiness: { ...readiness, expectedMeals, uniqueMealCount, dayCount, absentTakes },
    // Déclarations de présence de la fenêtre (livrable 1.5), telles quelles :
    // l'écran les affiche et les modifie, il ne les recalcule pas.
    presenceDeclarations,
