...
export async function authFetch(url, options = {}) {
  ...
  // Lecture FRAÎCHE forcée (opt-in) : court-circuite le cache SWR. Indispensable
  // quand l'appelant a besoin de l'état courant en base (ex. ingrédients reliés
  // pour la déduction de stock à la cuisson), pas d'une réponse de ≤ 24 h.
  if (options.noCache) {
    const entry = await networkFetch(url, options, session)
    if (entry.ok) { GET_CACHE.set(key, entry); persist() }
