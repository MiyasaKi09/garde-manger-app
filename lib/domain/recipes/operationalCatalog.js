...
//
// CE QUE LE RETRAIT N'A PAS FAIT À LUI SEUL, et qui a été fait depuis. Il ne
// sortait pas le corpus du bundle : `lib/domain/recipes/canonicalCatalog.js`
// importait encore les deux mêmes fichiers, et `app/_pricing/estimations.js`
// l'importait pour la seule `coutDeCarte`. Le §9.3 du plan vise nommément CE
// fichier-ci ; la sortie du bundle était le jalon J1 de la phase 5. Elle est
// faite : `coutDeCarte` vit maintenant dans `lib/domain/recipes/coutDeCarte.js`,
// qui n'importe aucun JSON, et plus aucune entrée Next n'atteint le corpus —
// mesuré, `.next/server` passe de 14 418 625 à 8 785 969 octets, et
// `tests/recipes/corpusHorsBundle.test.js` parcourt le graphe d'imports pour
// interdire le retour de l'arête.
//
...
export function operationalRecipeCards(recipes = []) {
  return recipes.map((recipe) => ({
    ...
    // La date de versement, telle que la RPC ÉDITORIALE la publie
    // (20260919140000). `null` veut dire « on ne sait pas quel jour cette
    // recette est entrée au catalogue », ce qui est le cas de 706 des 754 : la
    // carte ne le devine pas, et l'écran « Nouveautés » ne la montre pas.
    // La RPC opérationnelle, elle, ne publie pas ce champ — le planificateur
    // n'a pas à savoir qu'une recette est nouvelle.
    poured_on: recipe.pouredOn ?? null,
