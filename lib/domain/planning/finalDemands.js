...
/** Consigne de réchauffage d'un « reste compatible » servi à une prise support. */
function reheatSupportInstruction(leftover) {
  const portions = Number(leftover.portions) || 1
  const count = String(round(portions, 2)).replace('.', ',')
  return `Réchauffer ${count} portion${portions > 1 ? 's' : ''} de « ${leftover.name} » et servir.`
}
...
export function buildFinalDemandModel({
  ...
  // Déclarations de `public.meal_presence` (livrable 1.5). Elles ne servent
  // pas ici à retirer des assiettes — c'est `buildPersonalizedMeals` qui ne les
  // émet pas — mais à VÉRIFIER, juste avant de composer la liste de courses,
  // qu'aucune assiette déclarée absente n'a laissé de demande derrière elle.
  presence = [],
}) {
  ...
  // « Reste compatible » (§13) : une prise support nourrie par une portion de
  // plat cuisiné réserve cette portion SUR SON CRÉNEAU, exactement comme un
  // créneau principal. Le pool d'où vient la portion n'expose que des plats
  // qu'aucun créneau principal n'utilise : aucune portion ne peut être promise
  // deux fois. Les portions des membres d'un même créneau s'additionnent.
  const supportLeftovers = []
  for (const [slotKey, meals] of supportBySlot) {
    if (slotStates[slotKey]?.consumed) continue
    const fed = meals.filter((meal) => meal.portion_details?.leftover?.cooked_dish_id != null)
    if (!fed.length) continue
    const byDish = new Map()
    for (const meal of fed) {
      const leftover = meal.portion_details.leftover
      const key = String(leftover.cooked_dish_id)
      const current = byDish.get(key) || { ...leftover, portions: 0 }
      current.portions += Number(leftover.portions) || 0
      byDish.set(key, current)
    }
    for (const leftover of byDish.values()) {
      if (!(leftover.portions > 0)) continue
      supportLeftovers.push({
        slot_key: slotKey,
        meal_date: fed[0].meal_date,
        meal_type: fed[0].meal_type,
        cooked_dish_id: leftover.cooked_dish_id,
        name: leftover.name,
        canonical_recipe_code: leftover.canonical_recipe_code || null,
        portions: round(leftover.portions, 3),
        expires_on: leftover.expires_on || null,
      })
    }
  }
  for (const leftover of supportLeftovers) {
    reservations.push({
      slot_key: leftover.slot_key,
      cooked_dish_id: leftover.cooked_dish_id,
      ingredient_name: leftover.name,
      reserved_quantity: leftover.portions,
      reserved_unit: 'portion',
      needed_quantity: leftover.portions,
      needed_unit: 'portion',
      metadata: {
        allocation_strategy: 'cooked_dish_fefo',
        support: true,
        needed_on: leftover.meal_date,
        expires_on: leftover.expires_on,
      },
    })
  }
  ...
  // ── PRÉSENCE : LA PORTE D'ENTRÉE DE LA LISTE DE COURSES (livrable 1.5) ────
  //
  // La liste qui suit est l'agrégat des besoins des demandes retenues. Une
  // assiette déclarée absente n'a pas de demande, donc pas de besoin, donc pas
  // de quantité : les courses baissent d'autant, mécaniquement. Ce qui suit ne
  // retire rien — il VÉRIFIE. Une demande survivante pour une prise déclarée
  // absente ferait acheter des ingrédients pour un repas que personne ne prend,
  // et rien dans la liste ne le dirait. On échoue avec la personne et le
  // créneau nommés plutôt que de publier une liste silencieusement fausse.
  //
  // Seule exception, et c'est un invariant du dépôt (§17, planInvariants.js) :
  // un repas DÉJÀ CONSOMMÉ ne se réécrit pas. Une absence déclarée après coup
  // ne peut pas effacer un repas qui a eu lieu.
  if (presenceIndex.size) {
    for (const demand of demands) {
      if (!demand.person_name || !demand.meal_date || !demand.meal_type) continue
      if (demand.status === 'consumed') continue
      const membre = { id: demand.household_member_id, name: demand.person_name }
      if (!presenceIndex.absent(membre, demand.meal_date, demand.meal_type)) continue
      throw new Error(`final_demand_absent_member:${demand.slot_key}:${demand.person_name}`)
    }
  }
  ...
  const leftoverBySupportSlot = new Map(supportLeftovers.map((item) => [item.slot_key, item]))
  const supportSlots = [...supportBySlot.entries()].map(([slotKey, meals]) => {
    ...
    const leftover = leftoverBySupportSlot.get(slotKey) || null
    return {
      slot_key: slotKey,
      meal_date: meal.meal_date,
      meal_type: meal.meal_type,
      ...
      // Un support nourri par un reste porte son plat, comme un créneau
      // principal : c'est ce qui permet à l'écran et au RPC de le rattacher.
      ...(leftover ? { cooked_dish_id: leftover.cooked_dish_id } : {}),
      ...
      nutrition_source: leftover ? 'cooked_dish_stored_final_demands' : 'deterministic_supplement_forms',
      nutrition_confidence: 1,
      preparation: leftover ? {
        mode: 'support_leftover',
        cooked_dish_id: leftover.cooked_dish_id,
        exact_steps: [{ n: 1, instruction: reheatSupportInstruction(leftover) }],
        member_variants: [],
      } : {
        mode: 'support',
  ...
  // Un support qui demande un GESTE préalable — cuire des œufs durs, torréfier
  // un granola maison — porte une tâche de préparation, programmée la veille
  // pour un petit-déjeuner et le matin même pour une collation. Un « reste
  // compatible » (§13) porte à la place une tâche de réchauffage, le jour même :
  // le plat existe, il n'y a rien à préparer d'avance.
  const supportTasks = supportSlots.flatMap((slot) => {
    const meals = supportBySlot.get(slot.slot_key) || []
    const leftover = leftoverBySupportSlot.get(slot.slot_key) || null
    if (leftover) {
      const dueHour = slot.meal_type === 'pdj' ? '08:00:00' : '16:00:00'
      const due = zonedDateTimeToUtc(slot.meal_date, dueHour, householdTimeZone).toISOString()
      return [{
        task_key: `support-reheat-${slot.slot_key}`,
        slot_key: slot.slot_key,
        prep_date: slot.meal_date,
        prep_label: slot.meal_type === 'pdj' ? 'Petit-déjeuner' : 'Collation',
        title: `Réchauffer ${leftover.name}`,
        task_type: 'reheat_dish',
        earliest_start_at: zonedDateTimeToUtc(slot.meal_date, '06:00:00', householdTimeZone).toISOString(),
        due_at: due,
        safety_deadline_at: due,
        duration_min: 10,
        // Priorité 80, comme un créneau principal nourri par un reste : une
        // portion à consommer avant péremption passe devant une cuisson couverte.
        priority: 80,
        instructions: [{ n: 1, instruction: reheatSupportInstruction(leftover) }],
      }]
    }
    const items = meals.flatMap((meal) => meal.portion_details?.items || [])
    const eggCount = items
      .filter((item) => item.food === 'eggs')
      .reduce((sum, item) => sum + (Number(item.quantity) || 0), 0)
    const homemade = meals.map((meal) => meal.portion_details?.homemade).find(Boolean) || null
    if (!(eggCount > 0) && !homemade) return []
    ...
    const due = zonedDateTimeToUtc(prepDate, dueHour, householdTimeZone).toISOString()
    const earliest = zonedDateTimeToUtc(prepDate, '07:00:00', householdTimeZone).toISOString()
    const base = {
      slot_key: slot.slot_key,
      prep_date: prepDate,
    ...
    if (eggCount > 0) {
      tasks.push({
        ...base,
        task_key: `support-eggs-${slot.slot_key}`,
    ...
    if (homemade) {
      tasks.push({
        ...base,
        task_key: `support-${homemade.key}-${slot.slot_key}`,
        title: homemade.title,
        duration_min: Number(homemade.durationMin) || 15,
        instructions: [{
          n: 1,
          instruction: homemade.keepsDays > 1
            ? `${homemade.instruction} Se conserve ${homemade.keepsDays} jours.`
  ...
  // ── LE TOTAL NUTRITIONNEL DE LA SEMAINE, PAR PERSONNE (livrable 1.5) ──────
  //
  // Calculé sur les DEMANDES, c'est-à-dire sur les mêmes lignes qui viennent de
  // composer la liste de courses. Les deux chiffres ne peuvent donc pas
  // diverger : une assiette absente n'a pas de demande, elle ne pèse ni dans le
  // panier ni dans ce total. Les additionner ailleurs, à partir des repas ou
  // des journées, donnerait un second chiffre susceptible de dire autre chose.
  const weeklyNutritionByPerson = [...demands
    .filter((demand) => demand.person_name)
    .reduce((parPersonne, demand) => {
      const cle = demand.person_name
      const ligne = parPersonne.get(cle) || {
        person_name: demand.person_name,
        household_member_id: demand.household_member_id || null,
        mainSlots: new Set(),
        supportSlots: new Set(),
        nutrition: Object.fromEntries(NUTRIENTS.map((key) => [key, 0])),
      }
      // Une demande scindée (part de plat cuisiné + part fraîche) porte deux
      // lignes pour la MÊME assiette : on compte les créneaux, pas les lignes.
      const registre = ['dejeuner', 'diner'].includes(demand.meal_type) ? ligne.mainSlots : ligne.supportSlots
      registre.add(demand.slot_key)
      for (const key of NUTRIENTS) {
        ligne.nutrition[key] = round((ligne.nutrition[key] || 0) + (Number(demand.nutrition?.[key]) || 0), 2)
      }
      parPersonne.set(cle, ligne)
      return parPersonne
    }, new Map())
    .values()]
    .map(({ mainSlots, supportSlots, ...ligne }) => ({
      ...ligne,
      main_meals: mainSlots.size,
      support_takes: supportSlots.size,
    }))
  ...
  const model = {
    ...
    reservations,
    shoppingItems,
    // Ce que chacun mange réellement cette semaine, additionné sur les demandes
    // publiées (livrable 1.5).
    weeklyNutritionByPerson,
    resourcesBySlot,
    mainSlotSummaries,
    supportSlots,
    supportTasks,
