...
export function optimizeDailyPortions({
  ...
  const companionScales = steps(0.5, 2, 0.25)
  ...
  const lunchCompanionScales = lunchCompanion ? companionScales : [0]
  const dinnerCompanionScales = dinnerCompanion ? companionScales : [0]
    ...
    for (const snackScale of snackScales) {
      for (const lunchScale of lunchScales) {
        for (const dinnerScale of dinnerScales) {
          for (const lunchCompanionScale of lunchCompanionScales) {
            for (const dinnerCompanionScale of dinnerCompanionScales) {
          ...
          const companionPenalty = (
            (lunchCompanionScale ? (lunchCompanionScale - 1) ** 2 : 0)
            + (dinnerCompanionScale ? (dinnerCompanionScale - 1) ** 2 : 0)
          ) * 0.025
          const score = macroScore(total, target, lunchScale, dinnerScale) + energyDeviation ** 2 * 12
            + preferredRangePenalty + toleratedRangePenalty + supportPenalty + companionPenalty
          const candidate = {
            score,
            breakfastScale,
            snackScale,
  ...
  return best || {
    ...(bestFallback || {
      ...
      lunchCompanionScale: lunchCompanion ? 1 : 0,
      dinnerCompanionScale: dinnerCompanion ? 1 : 0,
...
function canonicalMeal({ slot, member, recipe, multiplier, plate, companionMultiplier, target, variantKind }) {
...
export function buildPersonalizedMeals({
  ...
  for (const [dayIndex, date] of dates.entries()) {
    ...
    for (const member of memberList) {
      ...
      const lunchPlate = buildMealPlate(lunchRecipe, constraints)
      const dinnerPlate = buildMealPlate(dinnerRecipe, constraints)
      ...
      // « Reste compatible » (§13) passe AVANT la rotation : un plat cuisiné qui
      // dort au frigo se mange, sinon il se jette. Un support déjà figé par un
      // plan antérieur n'est pas remplacé.
      ...
      const snackBase = snackLeftover ? leftoverSupport(snackLeftover, 'collation') : (rules.snack
        ...
        : null)
      // Un reste porte déjà sa nutrition (celle du plat) : elle ne se recalcule
      // pas depuis des `items` qu'il n'a pas.
      ...
      const optimized = optimizeDailyPortions({
        ...
        lunchCompanion: lunchPlate.components.length ? { nutrition: lunchPlate.nutritionPerServing } : null,
        dinnerCompanion: dinnerPlate.components.length ? { nutrition: dinnerPlate.nutritionPerServing } : null,
        rules: {
          ...rules,
          ...
          lunchCompanionMassPerServing: lunchPlate.massPerServing,
          dinnerCompanionMassPerServing: dinnerPlate.massPerServing,
