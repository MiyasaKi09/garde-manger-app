  ...
  const searchProducts = useCallback(async (query) => {
    ...
    try {
      ...
      const searches = await Promise.allSettled([
        // 1. CANONICAL FOODS
        supabase.from('canonical_foods').select(`
          id, canonical_name, category_id, subcategory, primary_unit,
          shelf_life_days_pantry, shelf_life_days_fridge, shelf_life_days_freezer, keywords
        `).or(searchTerms.map(term => 
          `canonical_name.ilike.%${term.replace(/[%_]/g, '\\$&')}%,keywords.cs.{${term}}`
        ).join(',')).limit(20),

        // 2. CULTIVARS
        supabase.from('cultivars').select(`
          id, cultivar_name, canonical_food_id, synonyms,
        ...
        // 3. GENERIC PRODUCTS
        supabase.from('generic_products').select(`
