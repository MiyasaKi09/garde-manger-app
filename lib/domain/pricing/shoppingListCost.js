...
export function computeShoppingListCost(items, index) {
  ...
  return {
     ...
     * propre quadrature sur ses propres lignes, et ‖a+b‖₂ ≤ ‖a‖₂ + ‖b‖₂ : la
     * fourchette d'achat est plus serrée que la somme des deux autres (à
     * égalité près, dans le cas dégénéré où toutes les lignes partagent le même
     * rapport surplus/besoin). Une interface qui vérifierait
     * `bas(achat) === bas(consommé) + bas(surplus)` trouverait un écart qui
