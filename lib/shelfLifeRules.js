...
export function calculateAdjustedExpiration(category, storageMethod, openedAt = new Date(), originalDLC = null) {
  ...
  // An explicit `null` means the storage method is forbidden. Only an
  // unknown method may fall back to the refrigerator rule.
  const hasExplicitMethod = Object.prototype.hasOwnProperty.call(rules, storageMethod);
  const daysToAdd = hasExplicitMethod ? rules[storageMethod] : rules.fridge;
  ...
  // Calculer nouvelle date (UTC-safe — setDate/getDate opèrent en heure locale
  // et peuvent décaler d'un jour selon le fuseau de l'utilisateur)
  const openedISO = openedAt instanceof Date
    ? openedAt.toISOString().split('T')[0]
    : String(openedAt).split('T')[0];
  const [oY, oM, oD] = openedISO.split('-').map(Number);
  const adjustedDate = new Date(Date.UTC(oY, oM - 1, oD + daysToAdd));
