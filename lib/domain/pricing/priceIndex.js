 ...
 * ne serait servi.
 */
function licenceComposee(entreesRetenues) {
...
export function buildPriceIndex(jeux, { today = null } = {}) {
  ...
  for (const [forme, liste] of candidats) {
    ...
    // Le conflit est enregistré avec de quoi le relire : le nom d'une tranche ne
    // suffit pas à savoir ce qui a été écarté, il faut voir les deux chiffres.
    const decrire = (c) => ({
      priceSetVersion: c.priceSetVersion,
      confidence: c.confidence,
      observedOn: c.provenance?.observed_on ?? null,
      perKgCentral: c.perKg.central,
      sourceCode: c.provenance?.source_code ?? null,
    })
    conflicts.push({
  ...
  return Object.freeze({
    ...
    derivedLicense: licenceComposee(retenues),
