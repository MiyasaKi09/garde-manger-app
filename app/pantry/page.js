// Remplacez la fonction transformée (autour de la ligne 145) par cette version corrigée :

// Transformer les données pour les adapter à l'interface
const transformed = (data || []).map(item => {
  // Déclaration initiale des IDs - ENLEVEZ CETTE PREMIÈRE DÉCLARATION
  ...
  let productInfo = null; // Ajoutez cette ligne si elle n'existe pas
  ...
  // GARDEZ SEULEMENT CETTE DÉCLARATION FINALE
  const canonicalFoodId = item.canonical_food_id
  ...
  const genericProductId = item.generic_product_id 
    ?? (productInfo?.type === 'generic' ? productInfo?.id : null)
    ?? item.generic_product?.id 
    ?? null;
  
  const derivedProductId = item.derived_product_id 
    ?? (productInfo?.type === 'derived' ? productInfo?.id : null)
    ?? item.derived_product?.id 
    ?? null;
