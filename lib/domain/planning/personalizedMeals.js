...
export function buildPersonalizedMeals({ plan, recipes, members, goals = [], constraints = {}, preservedMeals = [] }) {
  ...
  for (const [dayIndex, date] of dates.entries()) {
    ...
    for (const member of memberList) {
      ...
      const breakfastBases = rules.breakfast ? safeSupportTemplates([
        breakfastTemplate(dayIndex),
        ...Array.from({ length: 7 }, (_, offset) => breakfastTemplate(dayIndex + offset + 1)),
      ], constraints) : [null]
      const snackBases = rules.snack ? safeSupportTemplates(snackTemplates(dayIndex, target), constraints) : [null]
      ...
      for (const breakfastBase of breakfastBases.length ? breakfastBases : [null]) {
        for (const snackBase of snackBases.length ? snackBases : [null]) {
          ...
          const rank = (optimized.feasible ? 0 : 1000) + Number(optimized.score || 0)
          if (!selectedSupport || rank < selectedSupport.rank) {
            selectedSupport = { breakfastBase, snackBase, breakfast, snack, optimized, rank }
          }
        }
      }
      const {
        ...
        optimized: optimizeDailyPortions({
          target,
          breakfast: null,
          snack: null,
          lunch: lunchRecipe.nutritionPerServing,
