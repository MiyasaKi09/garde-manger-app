 ...
 *    `recipe.batchable`, `recipe.freezable`, `recipe.conservation`…) — la
 *    vérité éditoriale prime toujours ;
 * 2. à défaut, une dérivation DÉTERMINISTE de la composition réelle
 *    (ingrédients, catégories, techniques, textures). Jamais une inférence sur
 *    le nom du plat : « gratin de macaronis » et « salade de macaronis » ne se
 *    distinguent pas par leur titre mais par leurs ingrédients et leur
 *    technique.
 *
...
// Plats servis froids : les réchauffer n'a pas de sens.
const COLD_SERVICE = /salade|carpaccio|tartare|gaspacho|ceviche|taboule|vitello tonnato|froid|froide|crudite/
...
function deriveBatchability(recipe, { reheatQuality, servedCold }) {
  if (servedCold) return BATCHABILITY.LOW
...
export function buildRecipePlanningProfile(recipe) {
  ...
  const servedCold = COLD_SERVICE.test(fold(`${recipe?.family} ${recipe?.category}`))
  ...
  const derivedBatchability = deriveBatchability(recipe, { reheatQuality, servedCold })
  const batchability = declaredBatchable === null
    ? derivedBatchability
  ...
  // catégorisé, et au moins un signal de méthode ou de conservation.
  const hasCategorizedIngredient = (recipe?.exactIngredients || [])
    .some((ingredient) => !ingredient?.optional && fold(ingredient?.category))
  const judgeable = hasCategorizedIngredient && (
    techniques.length > 0
    || Boolean(recipe?.conservation)
    || declaredReheat != null
  ...
  return {
    ...
    // §14 — conservation et mutualisation
    reheatQuality,
    ...
    sources: {
      ...
      freezable: typeof recipe?.freezable === 'boolean' || Number(recipe?.freezerShelfLifeDays) > 0
        ? 'declared'
        : (recipe?.conservation ? 'conservation_text' : 'default'),
    },
  }
}
 ...
 * (§10). `null` quand la recette est éligible.
 */
export function batchRejectionReason(recipe) {
  const profile = recipePlanningProfile(recipe)
  if (!profile.documented) return 'composition_insuffisamment_documentee'
  if (profile.reheatQuality === REHEAT_QUALITY.POOR) {
    return profile.servedCold ? 'plat_servi_froid' : 'se_degrade_au_rechauffage'
  }
