...
export function resolveTokens(ingTokensRaw, candidates, stock) {
  ...
  if (best) {
    ...
    return {
      canonical_food_id: best.canonical_food_id,
      archetype_id: best.tier === 'archetype' ? best.archetype_id : null,
...
export function resolveIngredient(raw, { candidates, stock }) {
  ...
  const ingTokens = tokenize(nameForMatch, { dropUnits: true })
 ...
 */
export async function linkRecipeIngredients(supabase, recipe, { candidates, stock }) {
  ...
  for (const raw of list) {
    ...
    if (r.match_status !== 'unmatched') matched++
  ...
  return { total: list.length, matched, unmatched }
}
 ...
 */
export async function linkRecipesForUser(supabase, userId, { recipeId, all, onlyUnlinked } = {}) {
  ...
  if (!recipes?.length) return { recipes: 0, ingredients_total: 0, ingredients_matched: 0, match_rate: 0, details: [] }
  ...
  for (const recipe of targets) {
    const { total, matched, unmatched } = await linkRecipeIngredients(supabase, recipe, { candidates, stock })
    totalIng += total
    totalMatched += matched
    details.push({ recipe_id: recipe.id, title: recipe.title, total, matched, unmatched })
  }
