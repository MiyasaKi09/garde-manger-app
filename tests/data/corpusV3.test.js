...
describe('corpus culinaire V3', () => {
  it('contient les 356 plats réels et les graphes annoncés', () => {
    expect(corpus.corpus_version).toBe('v3-300-real-dishes')
    expect(corpus.recipes).toHaveLength(356)
    expect(corpus.food_form_graph).toHaveLength(720)
    expect(corpus.technique_graph).toHaveLength(431)
    expect(corpus.aroma_graph).toHaveLength(351)
  })
  ...
  it('conserve les identités nommées et les standards domestiques comme catégories distinctes', () => {
    ...
    expect(counts).toEqual({ named_traditional_dish: 306, domestic_standard: 50 })
  })
