...
_Généré le : Thu Sep  3 16:19:53 UTC 2026_
...
### cooked_dishes
 ...
 - canonical_recipe_execution_id :: uuid
...
### inventory_lots
 ...
 - container_count_initial :: smallint
 - barcode :: text
...
### nutrition_plan_meals
 ...
 - canonical_recipe_execution_id :: uuid
 - variant_kind :: text
 - portion_details :: jsonb default '{}'::jsonb NOT NULL
 - target_snapshot :: jsonb default '{}'::jsonb NOT NULL
 - demand_key :: text
 - execution_key :: text
 - constraints_snapshot :: jsonb default '{}'::jsonb NOT NULL
 - planning_status :: text default 'planned'::text NOT NULL
...
### nutrition_plan_shopping_items
 ...
 - needed_by :: date
 - exact_required_qty :: numeric
 - projected_surplus_qty :: numeric default 0 NOT NULL
 - purchase_decision :: jsonb default '{}'::jsonb NOT NULL
 - purchase_cost_low :: numeric
 - purchase_cost_central :: numeric
 - purchase_cost_high :: numeric
 - purchase_cost_currency :: text
 - purchase_cost_confidence :: text
 - purchase_cost_yield_known :: boolean
 - price_set_version :: text
 - price_set_reference_date :: date
...
### planned_demands
 ...
 - meal_type :: text NOT NULL
 - recipe_code :: text
 - requested_servings :: numeric NOT NULL
 - source_type :: text NOT NULL
 - source_id :: text
 - nutritional_target :: jsonb default '{}'::jsonb NOT NULL
 - constraints_snapshot :: jsonb default '{}'::jsonb NOT NULL
 - nutrition :: jsonb default '{}'::jsonb NOT NULL
 - support_items :: jsonb default '[]'::jsonb NOT NULL
 - status :: text default 'planned'::text NOT NULL
 - created_at :: timestamp with time zone default now() NOT NULL
...
### planned_productions
 ...
 - recipe_code :: text default ''::text NOT NULL
 - output_kind :: text default 'dish'::text NOT NULL
 - output_name :: text NOT NULL
 - planned_quantity :: numeric
 - planned_unit :: text
 - planned_portions :: numeric NOT NULL
 - storage_method :: text NOT NULL
...
### user_profiles
 ...
 - created_at :: timestamp with time zone default now() NOT NULL
 - updated_at :: timestamp with time zone default now() NOT NULL
 - food_budget_low :: numeric
 - food_budget_high :: numeric
 - food_budget_currency :: text
 - food_budget_period :: text
 - food_budget_set_on :: date
...
## Clés primaires
 ...
 - meal_feedback → (id)
 - meal_log → (id)
 - meal_plan_budget_overrides → (id)
 - meal_plan_slots → (id)
 - meal_plan_validation_issues → (id)
 - meal_plan_versions → (id)
 - meal_plans → (id)
 - meal_stock_deductions → (id)
 - meal_taste_feedback → (id)
 - member_food_preferences → (id)
 ...
 - planned_consumptions → (id)
 - planned_demands → (id)
 - planned_meals → (id)
 - planned_productions → (id)
...
## Clés étrangères
 ...
 - cooked_dishes.canonical_recipe_execution_id → recipe_executions.id  (constraint cooked_dishes_canonical_recipe_execution_id_fkey)
 - cooked_dishes.source_plan_version_id → meal_plan_versions.id  (constraint cooked_dishes_source_plan_version_id_fkey)
 - cooked_dishes.recipe_id → recipes.id  (constraint cooked_dishes_recipe_id_fkey)
 - cooked_dishes.planned_production_id → planned_productions.id  (constraint cooked_dishes_planned_production_id_fkey)
 ...
 - inventory_containers.lot_id → inventory_lots.id  (constraint inventory_containers_lot_id_fkey)
 ...
 - inventory_reservations.cooked_dish_id → cooked_dishes.id  (constraint inventory_reservations_cooked_dish_id_fkey)
 ...
 - meal_plan_budget_overrides.plan_version_id → meal_plan_versions.id  (constraint mpbo_plan_version_fk)
 ...
 - meal_plan_slots.recipe_execution_id → recipe_executions.id  (constraint meal_plan_slots_recipe_execution_id_fkey)
 ...
 - meal_taste_feedback.household_member_id → household_members.id  (constraint meal_taste_feedback_household_member_id_fkey)
 - member_food_preferences.household_member_id → household_members.id  (constraint member_food_preferences_household_member_id_fkey)
 ...
 - nutrition_plan_meals.canonical_recipe_execution_id → recipe_executions.id  (constraint nutrition_plan_meals_canonical_recipe_execution_id_fkey)
...
## Index
 ...
 - public.cooked_dishes → idx_cooked_dishes_canonical_identity : CREATE INDEX idx_cooked_dishes_canonical_identity ON public.cooked_dishes USING btree (user_id, canonical_recipe_code) WHERE (canonical_recipe_code IS NOT NULL)
 - public.cooked_dishes → idx_cooked_dishes_canonical_recipe_execution_id : CREATE INDEX idx_cooked_dishes_canonical_recipe_execution_id ON public.cooked_dishes USING btree (canonical_recipe_execution_id) WHERE (canonical_recipe_execution_id IS NOT NULL)
 ...
 - public.cooked_dishes → idx_cooked_dishes_planned_production_id : CREATE INDEX idx_cooked_dishes_planned_production_id ON public.cooked_dishes USING btree (planned_production_id) WHERE (planned_production_id IS NOT NULL)
 ...
 - public.cooked_dishes → idx_cooked_dishes_source_plan_version_id : CREATE INDEX idx_cooked_dishes_source_plan_version_id ON public.cooked_dishes USING btree (source_plan_version_id) WHERE (source_plan_version_id IS NOT NULL)
 ...
 - public.inventory_containers → inventory_containers_lot_ordinal_key : CREATE UNIQUE INDEX inventory_containers_lot_ordinal_key ON public.inventory_containers USING btree (lot_id, ordinal)
 - public.inventory_containers → inventory_containers_lot_status_idx : CREATE INDEX inventory_containers_lot_status_idx ON public.inventory_containers USING btree (lot_id, status, ordinal)
 - public.inventory_containers → inventory_containers_pkey : CREATE UNIQUE INDEX inventory_containers_pkey ON public.inventory_containers USING btree (id)
 - public.inventory_containers → inventory_containers_user_barcode_idx : CREATE INDEX inventory_containers_user_barcode_idx ON public.inventory_containers USING btree (user_id, barcode) WHERE (barcode IS NOT NULL)
 - public.inventory_containers → inventory_containers_user_id_idx : CREATE INDEX inventory_containers_user_id_idx ON public.inventory_containers USING btree (user_id)
 ...
 - public.inventory_lots → inventory_lots_user_barcode_idx : CREATE INDEX inventory_lots_user_barcode_idx ON public.inventory_lots USING btree (user_id, barcode) WHERE (barcode IS NOT NULL)
 - public.inventory_lots → inventory_lots_user_priced_idx : CREATE INDEX inventory_lots_user_priced_idx ON public.inventory_lots USING btree (user_id) WHERE (purchase_price IS NOT NULL)
 ...
 - public.inventory_reservations → idx_inventory_reservations_dish_active : CREATE INDEX idx_inventory_reservations_dish_active ON public.inventory_reservations USING btree (cooked_dish_id) WHERE (status = 'active'::text)
 ...
 - public.meal_plan_budget_overrides → idx_mpbo_plan_version : CREATE INDEX idx_mpbo_plan_version ON public.meal_plan_budget_overrides USING btree (plan_version_id) WHERE (plan_version_id IS NOT NULL)
 - public.meal_plan_budget_overrides → idx_mpbo_user_week : CREATE INDEX idx_mpbo_user_week ON public.meal_plan_budget_overrides USING btree (user_id, week_start DESC)
 - public.meal_plan_budget_overrides → meal_plan_budget_overrides_pkey : CREATE UNIQUE INDEX meal_plan_budget_overrides_pkey ON public.meal_plan_budget_overrides USING btree (id)
 - public.meal_plan_budget_overrides → mpbo_unique_week : CREATE UNIQUE INDEX mpbo_unique_week ON public.meal_plan_budget_overrides USING btree (user_id, week_start)
 ...
 - public.meal_plan_slots → idx_meal_plan_slots_execution_key : CREATE INDEX idx_meal_plan_slots_execution_key ON public.meal_plan_slots USING btree (plan_version_id, execution_key) WHERE (execution_key IS NOT NULL)
 ...
 - public.meal_plan_slots → idx_meal_plan_slots_recipe_execution : CREATE INDEX idx_meal_plan_slots_recipe_execution ON public.meal_plan_slots USING btree (recipe_execution_id) WHERE (recipe_execution_id IS NOT NULL)
 ...
 - public.meal_taste_feedback → idx_meal_taste_feedback_recipe : CREATE INDEX idx_meal_taste_feedback_recipe ON public.meal_taste_feedback USING btree (canonical_recipe_code) WHERE (canonical_recipe_code IS NOT NULL)
 - public.meal_taste_feedback → idx_meal_taste_feedback_user_date : CREATE INDEX idx_meal_taste_feedback_user_date ON public.meal_taste_feedback USING btree (user_id, meal_date DESC)
 - public.meal_taste_feedback → meal_taste_feedback_pkey : CREATE UNIQUE INDEX meal_taste_feedback_pkey ON public.meal_taste_feedback USING btree (id)
 - public.member_food_preferences → idx_member_food_preferences_user : CREATE INDEX idx_member_food_preferences_user ON public.member_food_preferences USING btree (user_id)
 - public.member_food_preferences → member_food_preferences_pkey : CREATE UNIQUE INDEX member_food_preferences_pkey ON public.member_food_preferences USING btree (id)
 - public.member_food_preferences → uq_member_food_preferences_subject : CREATE UNIQUE INDEX uq_member_food_preferences_subject ON public.member_food_preferences USING btree (household_member_id, subject_type, subject_value)
 ...
 - public.nutrition_plan_imports → uq_canonical_plan_import_week : CREATE UNIQUE INDEX uq_canonical_plan_import_week ON public.nutrition_plan_imports USING btree (user_id, date_range_start, date_range_end) WHERE (file_name = 'myko-canonical-v3'::text)
 ...
 - public.nutrition_plan_meals → idx_nutrition_plan_meals_canonical_code : CREATE INDEX idx_nutrition_plan_meals_canonical_code ON public.nutrition_plan_meals USING btree (canonical_recipe_code) WHERE (canonical_recipe_code IS NOT NULL)
 - public.nutrition_plan_meals → idx_nutrition_plan_meals_canonical_execution : CREATE INDEX idx_nutrition_plan_meals_canonical_execution ON public.nutrition_plan_meals USING btree (canonical_recipe_execution_id) WHERE (canonical_recipe_execution_id IS NOT NULL)
 - public.nutrition_plan_meals → idx_nutrition_plan_meals_demand_key : CREATE INDEX idx_nutrition_plan_meals_demand_key ON public.nutrition_plan_meals USING btree (import_id, demand_key) WHERE (demand_key IS NOT NULL)
 ...
 - public.nutrition_plan_shopping_items → idx_npsi_priced : CREATE INDEX idx_npsi_priced ON public.nutrition_plan_shopping_items USING btree (plan_version_id) WHERE (purchase_cost_central IS NOT NULL)
 ...
 - public.planned_consumptions → idx_planned_consumptions_production : CREATE INDEX idx_planned_consumptions_production ON public.planned_consumptions USING btree (planned_production_id) WHERE (planned_production_id IS NOT NULL)
 - public.planned_consumptions → idx_planned_consumptions_slot : CREATE INDEX idx_planned_consumptions_slot ON public.planned_consumptions USING btree (slot_id)
 - public.planned_consumptions → idx_planned_consumptions_version : CREATE INDEX idx_planned_consumptions_version ON public.planned_consumptions USING btree (plan_version_id)
 - public.planned_consumptions → planned_consumptions_pkey : CREATE UNIQUE INDEX planned_consumptions_pkey ON public.planned_consumptions USING btree (id)
 - public.planned_demands → idx_planned_demands_execution : CREATE INDEX idx_planned_demands_execution ON public.planned_demands USING btree (recipe_execution_id) WHERE (recipe_execution_id IS NOT NULL)
 - public.planned_demands → idx_planned_demands_member_date : CREATE INDEX idx_planned_demands_member_date ON public.planned_demands USING btree (household_member_id, meal_date) WHERE (household_member_id IS NOT NULL)
 - public.planned_demands → idx_planned_demands_slot_id : CREATE INDEX idx_planned_demands_slot_id ON public.planned_demands USING btree (slot_id)
 - public.planned_demands → idx_planned_demands_user_id : CREATE INDEX idx_planned_demands_user_id ON public.planned_demands USING btree (user_id)
 - public.planned_demands → idx_planned_demands_version_slot : CREATE INDEX idx_planned_demands_version_slot ON public.planned_demands USING btree (plan_version_id, slot_id)
 - public.planned_demands → planned_demands_pkey : CREATE UNIQUE INDEX planned_demands_pkey ON public.planned_demands USING btree (id)
 - public.planned_demands → planned_demands_plan_version_id_demand_key_key : CREATE UNIQUE INDEX planned_demands_plan_version_id_demand_key_key ON public.planned_demands USING btree (plan_version_id, demand_key)
 - public.planned_meals → planned_meals_pkey : CREATE UNIQUE INDEX planned_meals_pkey ON public.planned_meals USING btree (id)
 - public.planned_productions → idx_planned_productions_materialized : CREATE INDEX idx_planned_productions_materialized ON public.planned_productions USING btree (materialized_cooked_dish_id) WHERE (materialized_cooked_dish_id IS NOT NULL)
 - public.planned_productions → idx_planned_productions_recipe_execution_id : CREATE INDEX idx_planned_productions_recipe_execution_id ON public.planned_productions USING btree (recipe_execution_id) WHERE (recipe_execution_id IS NOT NULL)
 - public.planned_productions → idx_planned_productions_task : CREATE INDEX idx_planned_productions_task ON public.planned_productions USING btree (source_task_id) WHERE (source_task_id IS NOT NULL)
 - public.planned_productions → idx_planned_productions_version_status : CREATE INDEX idx_planned_productions_version_status ON public.planned_productions USING btree (plan_version_id, status)
 - public.planned_productions → planned_productions_pkey : CREATE UNIQUE INDEX planned_productions_pkey ON public.planned_productions USING btree (id)
 - public.planned_productions → planned_productions_plan_version_id_production_key_key : CREATE UNIQUE INDEX planned_productions_plan_version_id_production_key_key ON public.planned_productions USING btree (plan_version_id, production_key)
...
## Contraintes CHECK
 ...
 - inventory_containers_check ON inventory_containers : CHECK (((remaining_quantity >= (0)::numeric) AND (remaining_quantity <= initial_quantity)))
 - inventory_containers_empty_state_check ON inventory_containers : CHECK (((status <> ALL (ARRAY['empty'::text, 'discarded'::text])) OR (remaining_quantity = (0)::numeric)))
 - inventory_containers_initial_quantity_check ON inventory_containers : CHECK ((initial_quantity > (0)::numeric))
 - inventory_containers_ordinal_check ON inventory_containers : CHECK ((ordinal > 0))
 - inventory_containers_status_check ON inventory_containers : CHECK ((status = ANY (ARRAY['sealed'::text, 'open'::text, 'empty'::text, 'discarded'::text])))
 - inventory_lots_container_shape_check ON inventory_lots : CHECK (((is_containerized IS NOT TRUE) OR ((container_size > (0)::numeric) AND (NULLIF(btrim((container_unit)::text), ''::text) IS NOT NULL) AND (container_count_initial > 0))))
 ...
 - inventory_lots_purchase_price_check ON inventory_lots : CHECK ((((purchase_price IS NULL) AND (purchase_currency IS NULL) AND (purchase_price_basis IS NULL) AND (purchase_price_source IS NULL)) OR ((purchase_price IS NOT NULL) AND (purchase_currency IS NOT NULL) AND (purchase_price_basis IS NOT NULL) AND (purchase_price_source IS NOT NULL))))
 - inventory_lots_purchase_price_shape_check ON inventory_lots : CHECK ((((purchase_price IS NULL) OR (purchase_price >= (0)::numeric)) AND ((purchase_currency IS NULL) OR (purchase_currency ~ '^[A-Z]{3}$'::text)) AND ((purchase_price_basis IS NULL) OR (purchase_price_basis = ANY (ARRAY['lot'::text, 'container'::text, 'unit'::text]))) AND ((purchase_price_source IS NULL) OR (purchase_price_source = ANY (ARRAY['receipt'::text, 'manual_entry'::text, 'shopping_list'::text, 'barcode_scan'::text]))) AND ((purchase_price_basis IS DISTINCT FROM 'container'::text) OR (is_containerized IS TRUE))))
 ...
 - mpbo_bounds_check ON meal_plan_budget_overrides : CHECK ((((budget_low IS NULL) OR (budget_low > (0)::numeric)) AND ((budget_high IS NULL) OR (budget_high > (0)::numeric)) AND ((budget_low IS NULL) OR (budget_high IS NULL) OR (budget_low <= budget_high)) AND ((budget_low IS NOT NULL) OR (budget_high IS NOT NULL))))
 - mpbo_currency_check ON meal_plan_budget_overrides : CHECK ((currency ~ '^[A-Z]{3}$'::text))
 - mpbo_week_is_monday ON meal_plan_budget_overrides : CHECK ((EXTRACT(isodow FROM week_start) = (1)::numeric))
 ...
 - meal_taste_feedback_appreciation_check ON meal_taste_feedback : CHECK ((appreciation = ANY (ARRAY['loved'::text, 'acceptable'::text, 'to_adjust'::text, 'never_again'::text])))
 - meal_taste_feedback_meal_type_check ON meal_taste_feedback : CHECK ((meal_type = ANY (ARRAY['pdj'::text, 'dejeuner'::text, 'collation'::text, 'diner'::text])))
 - meal_taste_feedback_portion_fit_check ON meal_taste_feedback : CHECK (((portion_fit IS NULL) OR (portion_fit = ANY (ARRAY['too_small'::text, 'right'::text, 'too_large'::text]))))
 - member_food_preferences_appreciation_check ON member_food_preferences : CHECK ((appreciation = ANY (ARRAY['forbidden'::text, 'disliked'::text, 'avoided'::text, 'neutral'::text, 'liked'::text, 'favorite'::text, 'to_discover'::text])))
 - member_food_preferences_repeat_delay_days_check ON member_food_preferences : CHECK (((repeat_delay_days IS NULL) OR ((repeat_delay_days >= 0) AND (repeat_delay_days <= 365))))
 - member_food_preferences_source_check ON member_food_preferences : CHECK ((source = ANY (ARRAY['questionnaire'::text, 'feedback'::text, 'import'::text, 'inferred'::text])))
 - member_food_preferences_subject_type_check ON member_food_preferences : CHECK ((subject_type = ANY (ARRAY['ingredient'::text, 'recipe'::text, 'cuisine'::text, 'technique'::text, 'texture'::text, 'sensory_profile'::text, 'temperature'::text, 'richness'::text, 'vegetarian'::text, 'leftovers'::text])))
 ...
 - npsi_purchase_cost_check ON nutrition_plan_shopping_items : CHECK ((((purchase_cost_low IS NULL) AND (purchase_cost_central IS NULL) AND (purchase_cost_high IS NULL) AND (purchase_cost_currency IS NULL) AND (purchase_cost_confidence IS NULL) AND (purchase_cost_yield_known IS NULL) AND (price_set_version IS NULL) AND (price_set_reference_date IS NULL) AND (priced_at IS NULL)) OR ((purchase_cost_low IS NOT NULL) AND (purchase_cost_central IS NOT NULL) AND (purchase_cost_high IS NOT NULL) AND (purchase_cost_currency IS NOT NULL) AND (purchase_cost_confidence IS NOT NULL) AND (purchase_cost_yield_known IS NOT NULL) AND (price_set_version IS NOT NULL) AND (price_set_reference_date IS NOT NULL) AND (priced_at IS NOT NULL) AND (purchase_cost_low > (0)::numeric) AND (purchase_cost_low <= purchase_cost_central) AND (purchase_cost_central <= purchase_cost_high) AND (purchase_cost_currency ~ '^[A-Z]{3}$'::text) AND (purchase_cost_confidence = ANY (ARRAY['A'::text, 'B'::text])))))
 ...
 - planned_consumptions_one_source ON planned_consumptions : CHECK ((((((planned_production_id IS NOT NULL))::integer + ((cooked_dish_id IS NOT NULL))::integer) + ((lot_id IS NOT NULL))::integer) = 1))
 - planned_consumptions_positive_measure ON planned_consumptions : CHECK ((((portions IS NULL) OR (portions > (0)::numeric)) AND ((quantity IS NULL) OR (quantity > (0)::numeric)) AND ((portions IS NOT NULL) OR (quantity IS NOT NULL))))
 - planned_demands_check ON planned_demands : CHECK (((person_name IS NULL) OR (status = 'consumed'::text) OR (requested_servings <= (2)::numeric)))
 - planned_demands_meal_type_check ON planned_demands : CHECK ((meal_type = ANY (ARRAY['pdj'::text, 'dejeuner'::text, 'diner'::text, 'collation'::text])))
 - planned_demands_requested_servings_check ON planned_demands : CHECK ((requested_servings > (0)::numeric))
 - planned_demands_source_type_check ON planned_demands : CHECK ((source_type = ANY (ARRAY['fresh_recipe'::text, 'planned_production'::text, 'existing_dish'::text, 'support'::text, 'future_buffer'::text, 'consumed'::text])))
 - planned_demands_status_check ON planned_demands : CHECK ((status = ANY (ARRAY['planned'::text, 'consumed'::text, 'cancelled'::text])))
 - planned_productions_check ON planned_productions : CHECK ((use_by >= available_from))
 - planned_productions_planned_portions_check ON planned_productions : CHECK ((planned_portions > (0)::numeric))
 - planned_productions_status_check ON planned_productions : CHECK ((status = ANY (ARRAY['planned'::text, 'in_progress'::text, 'materialized'::text, 'cancelled'::text])))
 - planned_productions_storage_method_check ON planned_productions : CHECK ((storage_method = ANY (ARRAY['refrigerator'::text, 'freezer'::text])))
 ...
 - user_profiles_food_budget_check ON user_profiles : CHECK ((((food_budget_low IS NULL) OR (food_budget_low > (0)::numeric)) AND ((food_budget_high IS NULL) OR (food_budget_high > (0)::numeric)) AND ((food_budget_low IS NULL) OR (food_budget_high IS NULL) OR (food_budget_low <= food_budget_high)) AND (((food_budget_low IS NULL) AND (food_budget_high IS NULL)) = (food_budget_period IS NULL)) AND (((food_budget_low IS NULL) AND (food_budget_high IS NULL)) = (food_budget_currency IS NULL)) AND ((food_budget_period IS NULL) OR (food_budget_period = ANY (ARRAY['week'::text, 'month'::text]))) AND ((food_budget_currency IS NULL) OR (food_budget_currency ~ '^[A-Z]{3}$'::text))))
