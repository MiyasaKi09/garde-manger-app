...
export async function recomputeGeneratedNutrition(supabase, recipeId) {
  ...
  // Vérification qualité par rapport au CIQUAL (couverture, cohérence 4-4-9,
  // plausibilité) — calculée en même temps que la nutrition et persistée avec.
  try {
    const { data: vdata } = await supabase.rpc('verify_generated_recipe_nutrition', {
      gen_recipe_id_param: recipeId,
    })
    if (vdata) nutrition.verification = vdata
  } catch {
    // best-effort : la nutrition reste valable même si la vérification échoue
  }
