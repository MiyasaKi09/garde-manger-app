...
import { CAPACITE_NON_DECLAREE, resolveCookingCapacity } from './cookingCapacity'
...
import { cleCuisine, repartitionCuisines } from '../recipes/cuisineArbitrage'
import { countDenseCandidates, isProteinDense, resolveProteinDensityRequirement } from './proteinDensity'
import {
...
function consumeDishState(state, slot, { dish, recipe }, constraints, context) {
  ...
  return {
    ...
    weeklySummary: addToWeekSummary(state.weeklySummary, classifyRecipe(recipe), recipe, context?.proteinDensity?.floor ?? null),
...
// LES BORNES DE PRODUCTION SONT UNE CONSÉQUENCE DE LA CAPACITÉ DÉCLARÉE
// (livrable 2.2). Elles valaient 3 consommateurs par production et 2
// productions par plan, écrits ici en dur : des prudences posées quand
// personne ne savait de combien de temps le foyer disposait, alors même que le
// questionnaire lui demandait ses jours de cuisine et ses soirs rapides depuis
// des mois sans que rien ne les lise. Elles vivent maintenant dans
// `cookingCapacity.js`, qui les dérive de ce que les membres PRÉSENTS ont
// déclaré — et qui rend exactement ces deux nombres quand ils n'ont rien
// déclaré, de sorte qu'un foyer silencieux obtient le plan d'avant.
//
// Ce qui n'a pas changé, et qui reste la raison d'être d'une borne : sans elle,
// un garde-manger vide pousse le faisceau vers des semaines à quatre recettes,
// au détriment des règles de diversité hebdomadaires.

/**
 * La capacité déclarée refuse-t-elle les bases que ce candidat CUIRAIT à cette
 * date ? (livrable 2.2, règles R1 et R2.)
 *
 * Elle ne regarde que `cooked` : une base REPRISE ne coûte que sa minute de
 * remise en température (`BASE_REUSE_ACTIVE_MINUTES`) et n'est jamais refusée,
 * y compris un soir rapide — c'est exactement l'usage qu'on cherche à rendre
 * possible. Sans capacité déclarée, la fonction rend toujours `false` et ce
 * chemin n'existe pas.
 */
function basesRefusees(capacity, baseStock, date, slotBases) {
  const cuites = slotBases?.cooked?.length || 0
  if (!cuites) return false
  const capacite = capacity || CAPACITE_NON_DECLAREE
  if (!capacite.allowsCookingBaseOn(date)) return true
  const plafond = capacite.maxBasesPerSession
  if (plafond == null) return false
  // Bases déjà CUITES ce jour-là par cet état. `baseStock` porte la date de
  // cuisson de chaque base ; on ne compte donc ni les reprises, ni les bases
  // cuites un autre jour et encore valides.
  let dejaCuites = 0
  for (const entry of baseStock.values()) {
    if (entry?.cookedOn === date) dejaCuites += 1
  }
  return dejaCuites + cuites > plafond
}
...
export function classifyRecipe(recipe) {
  ...
  const classification = {
    ...
    mainStarch,
    // LA CUISINE NORMALISÉE PAR L'ARBITRAGE RELU — livrable 3.1.
    //
    // Elle valait `fold(recipe.cuisineOrigin)`, c'est-à-dire le libellé brut du
    // corpus replié. Le corpus en écrit 103 pour 754 recettes — 101 après
    // repli —, dont dix-neuf commencent par « France » : « France »,
    // « France / cuisine domestique internationale », « France (Bourgogne) »…
    // Comptés ainsi, un plat français et un autre plat français font deux
    // cuisines — la diversité de la semaine était mesurée sur une orthographe.
    // `cleCuisine` les compte une fois, d'après
    // `data/recipes/arbitrations/cuisines.json`, relu ligne à ligne.
    //
    // Ce que cela change dans le moteur, au-delà de la mesure : `cuisinesMin`,
    // le bonus de cuisine nouvelle et le regroupement sensoriel voient
    // désormais la même cuisine là où ils en voyaient deux. La semaine devient
    // donc plus variée en cuisines, pas seulement mieux comptée.
    cuisine: cleCuisine(recipe.cuisineOrigin),
    rich,
...
function weeklyTargets(constraints, totalSlots) {
  const balance = weeklyBalanceFor({
  ...
  // Plancher de densité protéique du foyer (livrable 1.4). Absent, les deux
  // clés valent `null` / 0 et tout ce qui les lit est inerte.
  const density = resolveProteinDensityRequirement(constraints.proteinDensity, totalSlots)
  return {
    ...balance,
    proteinDensityFloor: density.floor,
    proteinDenseMinSlots: density.minSlots,
  }
}

/**
 * `cuisines` est une Map depuis le livrable 3.1, et non plus un Set.
 *
 * Le plafond P13 (« aucune cuisine au-dessus de 40 % ») a besoin du NOMBRE de
 * créneaux par cuisine, pas seulement de la liste des cuisines vues. La Map
 * porte les deux : `.size` compte les cuisines distinctes exactement comme le
 * Set le faisait, `.has` répond comme avant, et `.get` donne la part. Un
 * second champ aurait fait deux comptes de la même chose, qui finissent par
 * diverger. Même raison pour `starches`, qui n'existait pas du tout : le
 * féculent dominant était mesuré dans le rapport de qualité et nulle part dans
 * le moteur.
 */
function emptyWeekSummary() {
  return { fish: 0, meat: 0, vegetarian: 0, redMeat: 0, fattyFish: 0, legumes: 0, cuisines: new Map(), starches: new Map(), proteins: new Map(), rich: 0, light: 0, proteinDense: 0 }
}

/**
 * `recipe` est passée en plus de sa classification parce que la densité
 * protéique se lit sur la nutrition de la recette, pas sur sa classification :
 * `classifyRecipe` range un plat par origine et par famille, elle ne connaît
 * pas le plancher du foyer, qui vient des objectifs de ses membres.
 */
function addToWeekSummary(current, c, recipe = null, densityFloor = null) {
  const summary = {
    ...current,
    cuisines: new Map(current.cuisines),
    starches: new Map(current.starches),
    ...
    if (densityFloor && isProteinDense(recipe, densityFloor)) summary.proteinDense++
    summary.cuisines.set(c.cuisine, (summary.cuisines.get(c.cuisine) || 0) + 1)
    // Un plat sans féculent n'en a pas : `mainStarch` vaut `null` et rien n'est
    // compté. Ranger ces plats sous une clé « aucun » en ferait une famille de
    // féculent comme une autre, qu'un plafond finirait par borner.
    if (c.mainStarch) summary.starches.set(c.mainStarch, (summary.starches.get(c.mainStarch) || 0) + 1)
    summary.proteins.set(c.mainProtein, (summary.proteins.get(c.mainProtein) || 0) + 1)
  return summary
}

/** Créneaux dont la protéine principale est un laitier ou un œuf — ce que P3 compte. */
function creneauxLaitiersOeufs(summary) {
  return DAIRY_EGG_PROTEIN_FAMILIES
    .reduce((total, famille) => total + (summary.proteins.get(famille) || 0), 0)
}
...
function weeklyDeficits(summary, targets) {
  ...
  // Plancher de densité protéique (livrable 1.4) : un manque devient un
  // avertissement nommé et chiffré, jamais une semaine refusée. La porte dure
  // d'`evaluateCandidate` n'est armée que si le vivier peut la franchir ; ce
  // déficit est ce qui reste visible quand elle ne l'est pas.
  add('protein_density_min', targets.proteinDenseMinSlots - summary.proteinDense)
...
/**
 * LES DÉPASSEMENTS DE PLAFOND DU LIVRABLE 3.1 — P2, P3, P13.
 *
 * MESURÉS À PART DES DÉFICITS, ET C'EST TOUT L'ENJEU DU LIVRABLE. Le §5 du plan
 * impose que ces plafonds soient, jusqu'au jalon C6, des « avertissements
 * mesurés, affichés, jamais des blocages silencieux ». Or `deficitWeight` — la
 * somme des `missing` de `weeklyDeficits` — est la DEUXIÈME clé de tri des
 * semaines finalistes, avant le score. Verser ces dépassements dans les
 * déficits aurait donc changé la semaine retenue sans rien refuser et sans que
 * personne puisse le voir : exactement la pénalité invisible que le commentaire
 * d'`evaluateCandidate` ci-dessous dit avoir écartée. Une première rédaction du
 * livrable le faisait ; elle est corrigée ici.
 *
 * Ils sont donc TOUJOURS calculés et TOUJOURS publiés — dans `issues` et dans
 * `objectiveScores.weeklyCaps.overruns` —, mais ils ne pèsent sur le choix de
 * la semaine QUE lorsque le régime est armé (`runBeamSearch`). Régime désarmé :
 * la semaine servie est exactement celle qu'on aurait servie sans ce livrable,
 * et le dépassement est écrit à côté d'elle.
 *
 * `missing` porte, comme dans `weeklyDeficits`, le nombre de créneaux EN TROP.
 * Le code nomme la famille, sans quoi « trois féculents dépassent » ne dirait
 * pas lesquels et personne ne saurait quoi corriger.
 */
function depassementsDesPlafonds(summary, targets) {
  const depassements = []
  const add = (code, missing) => { if (missing > 0) depassements.push({ code, missing }) }
  for (const [starch, count] of summary.starches) {
    add(`starch_cap_${starch}`, count - plafondDuFeculent(starch, targets))
  }
  add('dairy_egg_protein_cap', creneauxLaitiersOeufs(summary) - targets.dairyEggProteinMax)
  for (const [cuisine, count] of summary.cuisines) {
    add(`cuisine_cap_${cuisine}`, count - targets.cuisineMax)
  }
  return depassements
}
...
function quotaProgressScore(before, recipe, targets) {
  ...
  // Densité protéique (livrable 1.4). Barème aligné sur le quota poisson (18),
  // le plus fort de cette fonction : c'est le critère le plus en retard du
  // §2.3 — P4 à 3/7, 0/7, 1/7 — et le seul que la sélection puisse déplacer.
  // Le bonus cesse dès que le compte est atteint : au-delà, un plat dense ne
  // vaut pas mieux qu'un autre, et continuer à le récompenser reviendrait à
  // fabriquer une semaine de protéines sans que personne l'ait demandée.
  if (targets.proteinDensityFloor
    && before.proteinDense < targets.proteinDenseMinSlots
    && isProteinDense(recipe, targets.proteinDensityFloor)) score += 18
  return score
}
...
function evaluateCandidate(state, recipe, slot, constraints, targets, context, scale = 1, engage = null) {
  ...
  // livrable 3.1, moitié « refus » de la bascule.
  //
  // CE BLOC NE S'EXÉCUTE QU'ARMÉ. `context.capRegime.actif` vaut vrai au seul
  // jalon que le §5 du plan a écrit : la France passée sous 50 % du vivier
  // (`resolveRegimeDesPlafonds`). Le plan pose lui-même la raison — sur le
  // vivier d'aujourd'hui, 411 des 568 recettes publiables sont françaises après
  // normalisation des libellés, soit 72,4 % (57,6 % sur le corpus entier, dont
  // les 186 non publiables sont nettement moins françaises — c'est le vivier
  // REÇU qui compte ici). Un plafond de cuisine à 40 % comme REFUS n'a alors
  // mathématiquement aucune semaine à proposer : quatorze créneaux, cinq
  // français au plus, et rien de comparable pour occuper les neuf autres. Armer
  // les plafonds aujourd'hui produirait `no_feasible_plan`, c'est-à-dire aucune
  // semaine du tout, ce qui est pire que la semaine trop française d'aujourd'hui.
  //
  // CE QUI SE PASSE QUAND LE RÉGIME EST DÉSARMÉ, ET IL FAUT LE DIRE PRÉCISÉMENT.
  // Les dépassements ne disparaissent pas : `depassementsDesPlafonds` les
  // compte, les nomme, et ils sont publiés dans `issues` et dans
  // `objectiveScores.weeklyCaps.overruns`. Mais ils ne pèsent PAS sur le choix
  // de la semaine — c'est la différence entre eux et les déficits de quota, et
  // elle est écrite en toutes lettres dans l'en-tête de
  // `depassementsDesPlafonds`. Régime désarmé, ce bloc ne s'exécute pas et la
  // semaine servie est celle qu'on aurait servie sans lui.
  //
  // CE QUI CHANGE MALGRÉ TOUT DÈS CE LIVRABLE, et qu'il serait malhonnête de
  // passer sous silence : la cuisine que ce moteur lit est désormais la cuisine
  // NORMALISÉE (`classifyRecipe`). Le plan servi change donc, non par un
  // plafond, mais parce que « France » et « France / cuisine domestique
  // internationale » ont cessé d'être deux cuisines. C'est l'objet même du
  // livrable, et l'effet est mesuré dans `tests/planning/plafondsSemaine.test.js`.
  //
  // Un créneau ÉPINGLÉ (`fixedRecipeCode`) échappe à ces portes comme à toutes
  // les autres : l'utilisateur a décidé, on ne le corrige pas.
  if (!slot.fixedRecipeCode && context.capRegime?.actif) {
    if (classification.mainStarch
      && (week.starches.get(classification.mainStarch) || 0) >= plafondDuFeculent(classification.mainStarch, targets)) return null
    if (DAIRY_EGG_PROTEIN_FAMILIES.includes(classification.mainProtein)
      && creneauxLaitiersOeufs(week) >= targets.dairyEggProteinMax) return null
    if ((week.cuisines.get(classification.cuisine) || 0) >= targets.cuisineMax) return null
  }
  // PLANCHER DE DENSITÉ PROTÉIQUE — la contrainte de SÉLECTION du livrable 1.4.
  //
  // Elle n'écarte pas les plats peu denses en général : elle écarte ceux qui
  // rendraient le compte de la semaine inatteignable. Tant que les créneaux
  // restants suffisent à loger les repas denses encore dus, tout candidat
  // passe et c'est le bonus de `quotaProgressScore` qui oriente ; dès que le
  // nombre de créneaux restants tombe au niveau du reste à faire, seul un plat
  // dense peut encore les occuper.
  //
  // CE QUE CETTE FORME ÉVITE. Un refus permanent des plats peu denses viderait
  // le faisceau sur un corpus où ils sont majoritaires (médiane 0,050 g/kcal au
  // §1.4 mesuré) et rendrait `no_feasible_plan` — pire que tout ce que ce
  // livrable corrige. La porte n'est d'ailleurs armée (`context.proteinDensity
  // .gate`) que si le vivier compte assez de plats denses pour la franchir.
  if (!slot.fixedRecipeCode && context.proteinDensity?.gate && targets.proteinDensityFloor) {
    const remaining = Math.max(0, context.slotCount - state.slots.length)
    const owed = targets.proteinDenseMinSlots - week.proteinDense
    if (owed >= remaining && !isProteinDense(recipe, targets.proteinDensityFloor)) return null
  }
  ...
  // Trois derniers repas de la même cuisine — sur la cuisine NORMALISÉE
  // (livrable 3.1), sans quoi un bœuf bourguignon « France / cuisine domestique
  // internationale » suivi d'une salade lyonnaise « France (Lyon) » ne
  // comptaient pas comme deux plats français d'affilée.
  const cuisineRepeat = state.recipes.slice(-3)
    .filter((item) => cleCuisine(item.cuisineOrigin) === cleCuisine(recipe.cuisineOrigin)).length * 5
...
function selectProductionConsumers(state, recipe, slot, slots, slotIndex, dishPool, dishIndexes, constraints, targets, sessionBudget, context) {
  ...
  // LA CAPACITÉ DÉCLARÉE DÉCIDE OÙ ET COMBIEN (livrable 2.2). Trois refus, et
  // chacun porte le nom de ce que le foyer a dit :
  //   — un soir déclaré « rapide » ne porte AUCUNE production ;
  //   — quand des jours de cuisine sont déclarés, une production ne démarre
  //     que ces jours-là ;
  //   — une session porte au plus `maxProductionsPerSession` productions, et
  //     le plan au plus `maxPlanProductions`.
  // Rien de déclaré : `CAPACITE_NON_DECLAREE` rend les bornes d'avant (2 et 3)
  // et les trois refus ci-dessous ne se déclenchent jamais.
  const capacity = context.capacity || CAPACITE_NON_DECLAREE
  if (!capacity.allowsProductionOn(slot.date)) return null
  if (state.productionsUsed >= capacity.maxPlanProductions) return null
  // Productions déjà cuites CE JOUR-LÀ par cet état du faisceau. Comptées sur
  // `state.slots` plutôt que tenues dans un compteur de plus : le créneau
  // producteur porte déjà sa production, et un second compteur à maintenir en
  // cinq endroits est un second endroit où se tromper.
  const productionsCeJour = state.slots
    .filter((planned) => planned?.production && planned.date === slot.date).length
  if (productionsCeJour >= capacity.maxProductionsPerSession) return null
  ...
  const classification = classifyRecipe(recipe)
  let maxMeals = 1 + capacity.maxProductionConsumers
  ...
  // R3 du livrable 2.2 : une production nourrit JUSQU'À LA PROCHAINE SESSION
  // déclarée, pas au-delà — on recuisinera ce jour-là. `null` quand aucun jour
  // de cuisine n'est déclaré : la borne redevient alors le compte de trois
  // consommateurs d'avant ce livrable, porté par `maxMeals`.
  const horizon = capacity.productionHorizon(slot.date)
  for (let index = slotIndex + 1; index < slots.length && consumers.length + frozenConsumers.length < maxMeals - 1; index += 1) {
    ...
    if (horizon && candidate.date && candidate.date >= horizon) continue
    let storage = null
...
function runBeamSearch({ slots, recipes, inventoryLots, existingReservations, cookedDishes, existingDishReservations, constraints, beamWidth, context }) {
  ...
  for (let slotIndex = 0; slotIndex < slots.length; slotIndex += 1) {
    ...
    for (const state of beam) {
      ...
      for (const recipe of recipes) {
        ...
        // LES BASES SONT DU TRAVAIL DE CUISINE, elles aussi (livrable 2.2).
        // Cuire une sauce de vingt minutes un soir déclaré « rapide » est la
        // même contradiction qu'y lancer une production ; en cuire une
        // troisième un jour de session dépasse ce que ce jour porte. Reprendre
        // une base déjà cuite reste libre partout : c'est trois minutes, et
        // c'est le gain qu'on cherche. Refus SILENCIEUX au sens du faisceau —
        // le candidat n'est pas retenu sur CE créneau, il reste disponible
        // ailleurs, et un foyer qui n'a rien déclaré ne rencontre jamais ce
        // chemin (`basesRefusees` vaut alors toujours false).
        if (basesRefusees(context.capacity, state.baseStock, slot.date, evaluated.sharedBases)) continue
  ...
  const ranked = beam.map((state) => {
    ...
    const capOverruns = depassementsDesPlafonds(weeklySummary, targets)
    // Les dépassements de plafond ne pèsent sur le classement QUE si le régime
    // du livrable 3.1 est armé. Désarmés, ils sont rapportés et rien d'autre :
    // c'est la différence entre un avertissement et une pénalité muette.
    const deficitWeight = deficits.reduce((sum, item) => sum + item.missing, 0)
      + (context.capRegime?.actif ? capOverruns.reduce((sum, item) => sum + item.missing, 0) : 0)
    const audit = auditWeekRepetition({ slots: state.slots, rules: context.rules, history: context.history })
    return { ...state, weeklySummary, deficits, capOverruns, deficitWeight, audit }
  ...
  return {
    ...
    ...(budget ? { budget } : {}),
    // CAPACITÉ DE CUISINE (livrable 2.2). Clé absente quand rien n'est déclaré
    // — même doctrine que le budget. Quand elle est là, elle dit d'où viennent
    // les bornes employées : sans ces trois nombres à côté du plan, personne ne
    // peut relier « une seule production cette semaine » à ce qu'il a déclaré.
    ...(context.capacity?.declared
      ? {
        capacity: {
          sessionDates: [...context.capacity.sessionDates].sort(),
          quickDates: [...context.capacity.quickDates].sort(),
          conflicts: context.capacity.conflicts,
          // Les sessions déclarées qui n'ont AUCUN créneau après elles dans la
          // fenêtre : elles ne peuvent rien produire, et ce zéro doit se lire.
          sessionsWithoutLaterSlot: context.capacity.sessionsWithoutLaterSlot,
          maxPlanProductions: context.capacity.maxPlanProductions,
          maxProductionsPerSession: context.capacity.maxProductionsPerSession,
          maxBasesPerSession: context.capacity.maxBasesPerSession,
          productionsPlanned: best.slots.filter((slot) => slot.production).length,
        },
      }
      : {}),
    ...
    issues: [
      ...
      // Les dépassements de plafond voyagent dans les mêmes `issues` que les
      // déficits, avec la même sévérité, parce qu'ils disent la même chose à
      // l'écran : « la semaine servie dépasse une borne, de tant ». Ce qui les
      // distingue n'est pas leur affichage, c'est leur poids — nul tant que le
      // régime n'est pas armé.
      ...best.capOverruns.map((depassement) => ({ severity: 'warning', code: depassement.code, missing: depassement.missing })),
      ...audit.blockers,
      ...audit.warnings,
      ...budgetIssues,
      ...(sharedBases?.issues || []),
    ...
    objectiveScores: {
      ...
      // Compté à part de `weeklyRuleViolations` : une borne du foyer manquée et
      // un plafond du livrable 3.1 dépassé ne se corrigent pas de la même
      // manière, et les additionner ferait un chiffre dont on ne pourrait plus
      // rien déduire.
      weeklyCapOverruns: best.capOverruns.length,
      ...
      // LE RÉGIME DES PLAFONDS ET LA MESURE QUI L'A DÉCIDÉ — livrable 3.1.
      //
      // Publié systématiquement, et non « quand il y a quelque chose à dire ».
      // Un plafond qui n'apparaît que lorsqu'il mord se confond avec un plafond
      // absent le reste du temps ; ici, la ligne est toujours là et elle
      // distingue trois états : armé, désarmé parce que la France tient encore
      // plus de la moitié du vivier, désarmé parce que le foyer l'a demandé.
      weeklyCaps: {
        enforced: Boolean(context.capRegime?.actif),
        reason: context.capRegime?.raison ?? null,
        pivot: context.capRegime?.pivot ?? null,
        pivotShare: context.capRegime?.partPivot == null ? null : round(context.capRegime.partPivot, 4),
        pivotRecipes: context.capRegime?.recettesPivot ?? null,
        threshold: context.capRegime?.seuil ?? null,
        dominantCuisine: context.capRegime?.dominante ?? null,
        dominantShare: context.capRegime?.partDominante == null ? null : round(context.capRegime.partDominante, 4),
        poolCuisines: context.capRegime?.cuisines ?? null,
        poolSize: context.capRegime?.vivier ?? null,
        // Les libellés du vivier qu'aucune décision relue ne couvre. Liste vide
        // = l'arbitrage est complet. Non vide, la part de la France est fausse
        // d'autant, et il faut que cela se voie plutôt que de se deviner.
        unarbitratedLabels: context.capRegime?.libellesNonArbitres ?? [],
        // Les dépassements de la semaine RETENUE, nommés et chiffrés. Liste
        // vide = les quatre plafonds sont tenus. C'est cette liste que P2, P3
        // et P13 relisent, armée ou non : un avertissement qu'on ne publie pas
        // n'avertit personne.
        overruns: best.capOverruns.map(({ code, missing }) => ({ code, missing })),
      },
      weeklyActual: {
        ...
        // LE DÉTAIL QUE LES PLAFONDS DU LIVRABLE 3.1 EXIGENT. Un plafond qui ne
        // publie que « dépassé » ou « tenu » n'est pas un avertissement mesuré :
        // c'est un verdict. Ces trois lignes portent les comptes eux-mêmes,
        // cuisine par cuisine et féculent par féculent, de sorte que P2, P3 et
        // P13 se relisent sur la semaine servie sans recalculer quoi que ce soit.
        starchesBySlot: Object.fromEntries(best.weeklySummary.starches),
        cuisinesBySlot: Object.fromEntries(best.weeklySummary.cuisines),
        dairyEggProtein: creneauxLaitiersOeufs(best.weeklySummary),
        // Créneaux tenant le plancher de densité protéique (livrable 1.4), et
        // si la porte dure était armée. Deux chiffres à côté de leur cible :
        // sans eux, un plancher raté ne se distingue pas d'un plancher absent.
        proteinDense: best.weeklySummary.proteinDense,
        proteinDensityGate: Boolean(context.proteinDensity?.gate),
      },
    },
  }
}
...
export function generateClosedLoopPlan({
  ...
  const baseContext = {
    ...
    // CAPACITÉ DE CUISINE DÉCLARÉE (livrable 2.2), résolue UNE fois pour les
    // trois passes de la cascade. Elle borne les productions et les bases
    // cuites ; quand rien n'est déclaré, elle rend les bornes d'avant et le
    // plan est identique, octet pour octet.
    capacity: resolveCookingCapacity(constraints.cookingCapacity, slots),
    ...
    baseCatalog: recipes.some(usesSharedBase)
      ...
      : null,
    // Plancher de densité protéique du foyer (livrable 1.4), résolu UNE fois
    // pour les trois passes. `gate` dit si la porte dure d'`evaluateCandidate`
    // est armée : elle ne l'est que si le vivier compte au moins autant de
    // plats denses que de créneaux à tenir. Sinon l'exigence reste un bonus de
    // notation et un déficit rapporté — on ne rend jamais une semaine
    // infaisable pour un plancher que le corpus ne peut pas servir.
    // RÉGIME DES PLAFONDS DU LIVRABLE 3.1, résolu UNE fois pour les trois
    // passes de la cascade : il décrit le VIVIER, qui ne change pas d'une passe
    // à l'autre. `actif` dit si les plafonds écartent un plat ou se contentent
    // de le rapporter ; les chiffres qui l'accompagnent — part de la France,
    // cuisine dominante, libellés non arbitrés — sont publiés dans
    // `objectiveScores.weeklyCaps` pour qu'un refus soit toujours rattachable à
    // la mesure qui l'a armé.
    capRegime: (() => {
      const regime = resolveRegimeDesPlafonds({
        ...
        declare: typeof constraints.weeklyCaps?.enforce === 'boolean' ? constraints.weeklyCaps.enforce : null,
      })
      // Le vivier peut-il seulement servir une semaine sous ces plafonds ?
      // La question ne se pose que si le jalon est franchi ; sinon rien n'est
      // armé et compter coûterait une passe sur le corpus pour rien.
      const capacite = regime.actif
        ? capaciteDuVivier(recipes.map((recipe) => {
          const classification = classifyRecipe(recipe)
          return {
            cuisine: classification.cuisine,
        ...
        }), weeklyTargets(constraints, slots.length), slots.length)
        : null
      if (!capacite) return regime
      return capacite.suffisante
        ? { ...regime, capacite }
        : { ...regime, actif: false, raison: RAISONS_REGIME_PLAFONDS.VIVIER_TROP_ETROIT, capacite }
    })(),
    proteinDensity: (() => {
      const resolved = resolveProteinDensityRequirement(constraints.proteinDensity, slots.length)
      if (!resolved.floor || !resolved.minSlots) return { floor: resolved.floor, minSlots: resolved.minSlots, gate: false, denseCandidates: 0 }
      const denseCandidates = countDenseCandidates(recipes, resolved.floor)
      const asked = constraints.proteinDensity?.gate
      return {
        ...resolved,
        denseCandidates,
        gate: asked === false ? false : denseCandidates >= resolved.minSlots,
      }
    })(),
  }
  const input = { slots, recipes, inventoryLots, existingReservations, cookedDishes, existingDishReservations, constraints, beamWidth }

  const cascade = (context) => {
    const strict = runBeamSearch({ ...input, context: { ...context, repetitionMode: REPETITION_MODES.STRICT } })
    ...
    const core = runBeamSearch({ ...input, context: { ...context, repetitionMode: REPETITION_MODES.CORE } })
    ...
    const off = runBeamSearch({ ...input, context: { ...context, repetitionMode: REPETITION_MODES.OFF } })
  ...
  /**
   * La cascade des règles de répétition, plus le repli du plancher de densité.
   *
   * Extraite en fonction au livrable 3.1 pour que les plafonds puissent, à leur
   * tour, céder devant une semaine incomplète — et céder SEULS, sans relâcher
   * ni les règles de répétition ni le plancher. L'ordre des replis se lit de
   * l'intérieur vers l'extérieur : répétition, puis densité, puis plafonds.
   */
  const cascadeAvecPlancher = (baseContext) => {
  const avecPlancher = cascade(baseContext)
  if (avecPlancher.slots.length === slots.length || !baseContext.proteinDensity.gate) return avecPlancher

  /**
   * LE PLANCHER DE DENSITÉ CÈDE AVANT LA SEMAINE — livrable 1.4.
   *
   * La porte dure d'`evaluateCandidate` n'est armée que si le vivier compte
   * assez de plats denses ; cela ne garantit pas qu'un plat dense reste
   * admissible à CHAQUE créneau une fois les règles de répétition, les quotas
   * et les contraintes du foyer appliqués. Si la semaine ressort incomplète,
   * on la rejoue sans la porte et on NOMME le renoncement. Une semaine
   * infaisable serait pire que tout ce que ce livrable corrige.
   *
   * Cet axe hérite du statut du budget et du plafond de mutualisation : il ne
   * déclenche aucun relâchement des règles de répétition et n'est relâché par
   * aucun. Il cède seul, et il le dit.
   */
  const sansPlancher = cascade({ ...baseContext, proteinDensity: { ...baseContext.proteinDensity, gate: false } })
  if (sansPlancher.slots.length !== slots.length) {
    return sansPlancher.slots.length > avecPlancher.slots.length ? sansPlancher : avecPlancher
  }
  return {
    ...sansPlancher,
    status: 'review_required',
    issues: [
      ...
      {
        severity: 'blocker',
        code: 'protein_density_floor_relaxed',
        details: {
          floor: baseContext.proteinDensity.floor,
          minSlots: baseContext.proteinDensity.minSlots,
          denseCandidates: baseContext.proteinDensity.denseCandidates,
          reason: 'Aucune semaine complète ne tient le plancher de densité protéique avec le vivier disponible ; le plancher a été relâché, les autres règles non.',
        },
      },
    ],
  }
  ...
  const avecPlafonds = cascadeAvecPlancher(baseContext)
  if (avecPlafonds.slots.length === slots.length || !baseContext.capRegime?.actif) return avecPlafonds

  /**
   * LES PLAFONDS CÈDENT AVANT LA SEMAINE — livrable 3.1.
   *
   * Le majorant de `capaciteDuVivier` a déjà écarté le cas évident : un vivier
   * qui ne peut, dimension par dimension, remplir la semaine sous ces bornes.
   * Il ne dit rien de leur combinaison avec les règles de répétition, les
   * quotas carnés et les contraintes du foyer — trois plafonds tenables chacun
   * ne font pas une semaine tenable. Si la semaine ressort incomplète, on la
   * rejoue sans les plafonds et on NOMME le renoncement, avec les trois
   * plafonds concernés et leurs bornes.
   *
   * POURQUOI CE REPLI, ET PAS UN PLAFOND PLUS SOUPLE. Desserrer la borne pour
   * que la semaine passe reviendrait à gagner P2, P3 et P13 en changeant leur
   * définition — exactement ce que le plan s'interdit. Le plafond reste ce
   * qu'il est ; c'est son APPLICATION qui cède, une fois, bruyamment. Le
   * dépassement redevient alors un avertissement mesuré, celui de
   * `weeklyDeficits`, et les deux se lisent ensemble.
   *
   * Comme le budget, le plafond de mutualisation et le plancher de densité :
   * cet axe ne déclenche aucun relâchement des règles de répétition et n'est
   * relâché par aucun. Il cède seul, et il le dit.
   */
  const sansPlafonds = cascadeAvecPlancher({ ...baseContext, capRegime: { ...baseContext.capRegime, actif: false } })
  if (sansPlafonds.slots.length !== slots.length) {
    return sansPlafonds.slots.length > avecPlafonds.slots.length ? sansPlafonds : avecPlafonds
  }
  return {
    ...sansPlafonds,
    status: 'review_required',
    issues: [
      ...
      {
        severity: 'blocker',
        code: 'weekly_caps_relaxed',
        details: {
          pivot: baseContext.capRegime.pivot,
          pivotShare: baseContext.capRegime.partPivot == null ? null : round(baseContext.capRegime.partPivot, 4),
          threshold: baseContext.capRegime.seuil,
          capacity: baseContext.capRegime.capacite ?? null,
          reason: 'Aucune semaine complète ne tient les plafonds de féculent, de protéine laitière ou d’œuf et de cuisine avec le vivier disponible ; ces plafonds ont été relâchés, les autres règles non.',
        },
      },
    ],
  }
