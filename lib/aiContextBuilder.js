...
→ CONSIGNE : planifie ces restes sur les PREMIERS JOURS du planning, AVANT leur DLC (règle FIFO : le plus ancien d'abord). Un repas couvert par un reste = le reste tel quel (pas de nouvelle recette pour ce créneau), avec ses macros par portion comptées dans le total du jour.`)
  ...
  // Contraintes alimentaires strictes (allergies + régimes + interdits perso user_food_bans)
  const allergies = ctx.allergies || []
  const diets = ctx.diets || []
  const bans = ctx.bans || []
  const dislikes = ctx.dislikes || []
  if (allergies.length > 0 || diets.length > 0 || bans.length > 0 || dislikes.length > 0) {
    const constraintLines = []
    // Fusion allergies + bans (kind='ban') dans la section INTERDITS STRICTS
    const strictForbidden = [...new Set([...allergies, ...bans])]
    if (strictForbidden.length > 0) {
      constraintLines.push(`- Interdits stricts (allergies + interdits perso) : ${strictForbidden.join(', ')} → AUCUN plat, sauce, garniture ou sous-ingrédient ne doit en contenir, sous aucune forme.`)
    }
    if (diets.length > 0) {
      constraintLines.push(`- Régimes suivis : ${diets.join(', ')} → TOUS les plats proposés doivent les respecter.`)
    }
    // kind='dislike' : préférences négatives, listées séparément
    if (dislikes.length > 0) {
      constraintLines.push(`- À éviter (préférence négative) : ${dislikes.join(', ')} → à ne proposer qu'en dernier recours.`)
    }
...
/**
 * Allergies (user_allergies → canonical_foods), régimes (user_diets → diets)
 * et interdits/préférences personnalisés (user_food_bans).
 *
 * Exporté : utilisé par les routes routine pour transmettre les contraintes
 * aux Routines claude.ai via formatDietaryConstraints, et par buildAiContext
 * pour alimenter formatContextForPrompt (section CONTRAINTES ALIMENTAIRES STRICTES).
 *
 * Fail-soft sur user_food_bans : si la table n'existe pas encore (pre-migration),
 * bansRes.error sera défini → bans/dislikes = [] sans crash.
 */
export async function fetchDietaryConstraints(supabase, userId) {
  const [allergyRes, dietRes, bansRes] = await Promise.all([
    supabase
      .from('user_allergies')
    ...
    supabase
      ...
      .eq('user_id', userId),
    // Fail-soft : bansRes.error si la table n'existe pas encore (pre-migration)
    supabase
      .from('user_food_bans')
      .select('name, kind')
      .eq('user_id', userId),
  ...
  // Fail-soft : bansRes.error → liste vide (app reste fonctionnelle pre-migration)
  const bans = (!bansRes.error && bansRes.data)
    ? bansRes.data.filter(r => r.kind === 'ban').map(r => r.name).filter(Boolean)
    : []
  const dislikes = (!bansRes.error && bansRes.data)
    ? bansRes.data.filter(r => r.kind === 'dislike').map(r => r.name).filter(Boolean)
    : []

  return { allergies, diets, bans, dislikes }
}
 ...
 *
 * Accepte désormais bans (kind='ban' de user_food_bans, fusionnés avec allergies)
 * et dislikes (kind='dislike', listés séparément comme "à éviter").
 */
export function formatDietaryConstraints({ allergies = [], diets = [], bans = [], dislikes = [] } = {}) {
  const parts = []
  const strictForbidden = [...new Set([...allergies, ...bans])]
  if (strictForbidden.length) parts.push(`Interdits stricts (aucune trace, aucun sous-ingrédient) : ${strictForbidden.join(', ')}.`)
  if (diets.length) parts.push(`Régimes à respecter : ${diets.join(', ')}.`)
  if (dislikes.length) parts.push(`À éviter (préférence) : ${dislikes.join(', ')}.`)
  return parts.join(' ')
}
