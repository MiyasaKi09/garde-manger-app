...
import { conservationProfileFromCorpus } from '@/lib/domain/recipes/conservationProfile'
...
export function materializeRecipe(recipe, foodCatalog, { servings = recipe.servings } = {}) {
  ...
  for (const ingredient of recipe.ingredients || []) {
    ...
    exactIngredients.push({
      ...
      // LA BASE PARTAGÉE que cette ligne déclare (livrable 2.1), posée au corpus
      // par scripts/data/recipes/link-shared-bases.mjs depuis un arbitrage relu.
      // `lib/domain/planning/sharedBases.js` la lit sous ce nom exact, et le
      // chemin base la sert déjà sous le même (materializeOperationalRecipe,
      // qui applique la même mise à l'échelle de `requiredQuantity`). Sans ce
      // passage, les liens seraient posés au corpus et invisibles au solveur sur
      // le chemin JSON — le rapport de qualité mesurerait 0 là où le corpus en
      // porte 77. `null` quand la ligne ne déclare rien : absence, pas oubli.
      component: ingredient.component ? {
        ...ingredient.component,
        requiredQuantity: Number.isFinite(Number(ingredient.component.requiredQuantity))
          ? Number(ingredient.component.requiredQuantity) * scale
          : null,
      } : null,
    })
  }
  ...
  return {
    ...
    // Le profil de conservation DÉCLARÉ (durée réfrigérateur, consommation
    // immédiate, congélation, service froid) : la seule chose que le moteur
    // lit pour produire, congeler ou réchauffer. La prose reste exposée pour
    // l'affichage, jamais pour décider.
    conservationProfile: conservationProfileFromCorpus(recipe.conservation_profile),
