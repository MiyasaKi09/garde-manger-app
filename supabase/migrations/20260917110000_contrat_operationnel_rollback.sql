-- Rollback du contrat opérationnel (20260917110000).
--
-- IL REMET LA RPC DANS SON ÉTAT DU 15 JUILLET, mot pour mot : le corps
-- ci-dessous est extrait de 20260715190000_v3_operational_recipe_api.sql, sans
...
--   1. la projection des quatre champs (la RPC redevient celle d'origine) ;
--   2. la fonction de traduction du profil de conservation, qui n'a plus
--      d'appelant une fois la RPC revenue en arrière ;
--   3. les colonnes déclaratives et leurs contraintes.
--
-- CE QU'IL COÛTE, ET QU'IL FAUT SAVOIR AVANT DE LE JOUER. Retirer
-- `catalog.food_forms.origin` et `culinary.recipe_versions.conservation_profile`
-- EFFACE les valeurs que 20260917111000 y a écrites. Elles se rechargent depuis
-- les artefacts versionnés du dépôt (scripts/data/out/recipe-food-catalog.json,
-- data/recipes/corpus-v3.json), donc rien n'est perdu pour de bon — mais rien
-- n'est récupérable depuis la base seule. Et tant que
-- lib/domain/recipes/operationalCatalog.js porte ses deux raccords JSON
-- (lignes 13 et 25), l'application continue de servir origine et conservation
-- depuis le corpus embarqué : ce rollback est donc jouable sans casser le
-- végétarien EN L'ÉTAT. Après le retrait des raccords (livrable 0b.4), il ne
-- l'est plus, et c'est le retrait des raccords qu'il faudra défaire d'abord.
...
DROP FUNCTION IF EXISTS culinary.conservation_profile_contract(jsonb);

DROP INDEX IF EXISTS catalog.idx_food_forms_origin;

ALTER TABLE culinary.recipe_versions
  DROP CONSTRAINT IF EXISTS recipe_versions_conservation_profile_check;
ALTER TABLE culinary.recipe_versions
  DROP COLUMN IF EXISTS conservation_profile;
