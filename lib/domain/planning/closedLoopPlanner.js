 ...
 */
function selectProductionConsumers(state, recipe, slot, slots, slotIndex, dishPool, dishIndexes, constraints, targets) {
  if (slot.fixedRecipeCode) return null
  if (state.productionsUsed >= MAX_PLAN_PRODUCTIONS) return null
  const activeMinutesSaved = (Number(recipe.prepMinutes) || 0) - REHEAT_ACTIVE_MINUTES
  ...
  const consumers = []
  for (let index = slotIndex + 1; index < slots.length && consumers.length < maxMeals - 1; index += 1) {
    const candidate = slots[index]
    if (!candidate.date || candidate.date > useBy) continue
  ...
  return { consumers, useBy, bonus: round(consumers.length * activeMinutesSaved * MUTUALISATION_WEIGHT, 4) }
}
...
export function generateClosedLoopPlan({ slots, recipes, inventoryLots = [], existingReservations = [], cookedDishes = [], existingDishReservations = [], constraints = {}, beamWidth = 24 }) {
  ...
  let beam = [{ score: 0, availability: buildAvailability(inventoryLots, existingReservations), recipes: [], slots: [], usedCodes: new Set(), weeklySummary: emptyWeekSummary(), dishPortionsUsed: new Map(), productionCovers: new Map(), productionsUsed: 0 }]
  ...
  for (let slotIndex = 0; slotIndex < slots.length; slotIndex += 1) {
    ...
    for (const state of beam) {
      ...
      for (const recipe of recipes) {
        ...
        // mutualisation explicite = temps actif économisé).
        const strategy = selectProductionConsumers(state, recipe, slot, slots, slotIndex, dishPool, dishIndexes, constraints, targets)
        if (!strategy) continue
        const scale = strategy.consumers.length + 1
        const batchEvaluated = evaluateCandidate(state, recipe, slot, constraints, targets, scale)
        ...
        expanded.push({
          score: state.score + batchEvaluated.score + strategy.bonus,
