...
export function parsePersonSheet(worksheet, personName, fileName) {
  ...
  for (let i = 0; i <= header.headerIndex; i++) {
    ...
    if (isPdjBanner(row)) {
      ...
      pdjDesc = descCell ? str(descCell) : fullText
...
export function parseRecipesSheet(worksheet) {
  ...
  for (let i = 0; i < Math.min(5, rows.length); i++) {
    ...
    if (hasRecette || hasIngr) { headerIdx = i; break }
  }
  ...
  for (let i = 0; i < headerRow.length; i++) {
    ...
    if (/r[ée]chauff/i.test(s)) colMap.reheat = i
    if (/instruction|préparation/i.test(s)) colMap.instructions = i
  ...
  for (let i = headerIdx + 1; i < rows.length; i++) {
    ...
    batchRecipes.push({
      ...
      reheat: colMap.reheat != null ? str(row[colMap.reheat]) : null,
      instructions: colMap.instructions != null ? str(row[colMap.instructions]) : null
    })
  }
