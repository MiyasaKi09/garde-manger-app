 ...
 *
 * `dessert` a rejoint la liste (lot « pâtisserie servie en dîner ») : c'est un
 * composant de FIN DE REPAS, jamais un accompagnement au sens starch/vegetables.
 * Il n'est proposé que par un mécanisme distinct — voir `buildDessertComponent`
 * dans `mealComposition.js` — jamais par `componentToAdd`, qui reste dédié à ce
 * qui COMPLÈTE structurellement un plat principal amputé.
 */
export const COMPOSANTES_ADJOIGNABLES = Object.freeze(['starch', 'vegetables', 'dessert'])
...
/**
 * Une recette est-elle un dessert AU SENS DE LA RÈGLE DURE ? Autrement dit :
 * l'éditeur du corpus a-t-il DÉCLARÉ `plate: { role: 'dessert' }` sur cette
 * recette ? C'est la source de vérité utilisée par le filtre du solveur
 * (closedLoopPlanner.js) pour interdire qu'une pâtisserie occupe un créneau
 * `dejeuner`/`diner` comme plat principal.
 *
 * Un choix DÉLIBÉRÉ : la fonction ne retombe pas sur la dérivation par
 * catégorie/nom. Deux raisons :
 *
 * 1. Le contrat avec l'agent des rôles est explicite — les 20 recettes
 *    sucrées reçoivent `plate.role='dessert'` en ce moment même. La règle
 *    dure prend effet dès que la déclaration existe, et pas avant. C'est
 *    l'inertie promise au tech-lead.
 *
 * 2. La dérivation contient des motifs de nom volontairement larges (« tarte
 *    aux » capture aussi une tarte aux poireaux), utiles pour l'assiette où
 *    la question est « ce plat accueille-t-il un féculent », inadaptés pour
 *    une règle qui exclut absolument un plat des créneaux de repas. Se
 *    reposer sur elle ferait régresser des recettes salées légitimes.
 *
 * `derivePlateRole` reste appelable directement quand on VEUT explicitement
 * l'inférence — c'est ce que fait `buildMealPlate` pour composer l'assiette
 * quotidienne, où le coût d'une erreur est nul (au pire, un accompagnement
 * inutile est proposé, pas une exclusion silencieuse).
 */
export function isDessertRecipe(recipe) {
  const declared = declaredPlateRole(recipe)
  return declared ? declared.role === PLATE_ROLES.DESSERT : false
}
