import { refrigeratorShelfLifeDays } from './cookingSessions'
 ...
 * DÉCLARÉE (`shelfLifeDays`), sinon les heures de son profil de conservation
 * déclaré (`conservationProfile.fridgeHours`), la même règle que celle des
 * productions (cookingSessions.refrigeratorShelfLifeDays). Rien de déclaré :
 * pas de base partagée. Jamais une durée lue dans un texte, jamais une durée
 * devinée d'après le nom de la base, jamais la constante de trois jours des
 * plats cuisinés — elle a servi de garde à un pan bagnat de 24 heures.
 *
...
/**
 * Fenêtre de conservation d'une base au réfrigérateur, en jours : la durée
 * DÉCLARÉE par sa recette (`shelfLifeDays`, sinon son profil de conservation),
 * la même règle que `productionShelfLifeDays` du solveur — ou null quand rien
 * n'est déclaré. L'ancien repli sur la règle commune des plats cuisinés
 * (3 jours) a été retiré : une base dont on ne connaît pas la garde n'entre
 * pas au catalogue des bases partagées, exactement comme une base dont on ne
 * connaît pas les minutes.
 */
export function baseShelfLifeDays(baseRecipe) {
  return refrigeratorShelfLifeDays(baseRecipe)
}
...
export function buildSharedBaseCatalog(recipes = [], baseRecipes = []) {
  ...
  for (const recipe of [...(baseRecipes || []), ...(recipes || [])]) {
    ...
    // Ni minutes, ni conservation : rien à décider (SHARED_BASE_NOTES.UNKNOWN
    // côté plat). Une base sans garde déclarée n'est pas une base à trois
    // jours, c'est une base dont on ne sait pas la garde.
    const shelfLifeDays = baseShelfLifeDays(recipe)
